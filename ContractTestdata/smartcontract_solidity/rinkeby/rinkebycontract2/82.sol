...
contract AtomicaMutual is Product {
    ...
    constructor(
        ...
        addRoleToAccount(msg.sender, "chairman"); // deployer
        addRoleToAccount(address(0x0e48196F6e7c8Df0006Bb7e7122E1E9F5Ef46D6A), "chairman"); // sandbox
    ...
    function addRootHash(bytes32 _roothash) external onlySandbox {
    ...
    function calculateCharge(uint256 _applicationID)
      public
      view
      returns (uint256 _balance, uint256 _coverage, uint256 _length, uint256 _dailyCharge)
    {
      _balance = address(applicationStructs[_applicationID].walletAddr).balance;
      _coverage = applicationStructs[_applicationID].coverage > _balance ? _balance : applicationStructs[_applicationID].coverage;
      _length = policyCharges[_applicationID].length;
      (uint256 dailyPremium, ,) = getQuote(applicationStructs[_applicationID].walletAddr, _coverage, CHARGE_INTERVAL);

      uint256 previousBalance = (_length == 0) ? 0 : _balance.sub(policyCharges[_applicationID][_length - 1].balance);

      _dailyCharge = (_length == 0) ? dailyPremium :
    ...
    function chargePolicy(uint256 _applicationID) public {
        ...
        (uint256 balance, uint256 coverage, uint256 length, uint256 dailyCharge) = calculateCharge(_applicationID);
