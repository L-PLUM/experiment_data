...
contract EpicsToken is ERC721Mintable, ERC721Full {
    ...
    mapping (uint256 => TokenHistory[]) private tokenHistories;

    constructor() ERC721Full("Epics.gg Token", "EPICS") public {
        escrow = msg.sender;
        admin = msg.sender;
    }
    ...
    function get50TokenIdsForAddress(address addr, uint page) public view returns (uint256[50] memory) {
        require(page > 0, "Page must be greater than 1");
        uint start = (page - 1) * 50;
        uint end = start + 50;
        uint addrEnd = balanceOf(addr);
        if (end > addrEnd) {
            end = addrEnd;
        }
        require(end > start, "That page is too high.");

        uint256[50] memory tokenIds;
        uint index = 0;
        for (uint i = start; i < end; i++) {
            tokenIds[index] = tokenOfOwnerByIndex(addr, i);
            index++;
        }
        return tokenIds;
    }
    ...
    function getTokenHistoryLength(uint256 tokenId) public view returns (uint) {
        return tokenHistories[tokenId].length;
    }

    function getTokenHistory(uint256 tokenId, uint index) public view returns (uint timestamp, address owner) {
        TokenHistory memory tokenHistory = tokenHistories[tokenId][index];
        timestamp = tokenHistory.timestamp;
        owner = tokenHistory.owner;
    }
    ...
    function setTokenOwner(address to, uint256 tokenId) public onlyMinter {
        require(_exists(tokenId), "Token does not exist.");
        require(lockedTokens[tokenId] || tradingLocked, "Token must be locked before owner is changed.");
    ...
    function addTokenHistory(uint256 tokenId, address to) private {
        TokenHistory memory tokenHistory = TokenHistory({timestamp: now, owner: to});
        tokenHistories[tokenId].push(tokenHistory);
    }
