...
contract Nftprice is Ownable, SignerRole {
    ...
    uint public minBalance = 8000000000000000;
    ...
    function buy_(address[3] memory addrs, uint[4] memory uints, uint8 v, bytes32 r, bytes32 s) public payable onlySigner{
        buy(
    ...
    function buy(Order memory order, Sig memory sig)
        internal
    {
        require(msg.value == order.price, "Not proper value send");
        bytes32 hash = requireValidOrder(order, sig);
        ERC721(order.asset).transferFrom(order.maker, msg.sender, order.tokenId);
    ...
    function updateLimit(uint _limit) external onlySigner{
        limit = _limit;
    }

    function updateMinBalance(uint _minBalance) external onlySigner{
