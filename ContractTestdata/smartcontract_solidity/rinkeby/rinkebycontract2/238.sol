...
contract TradeableERC721Token is ERC721Full {
  ...
  mapping (bytes32 => bool) public validURI;
  ...
  // Owner of contract can give alternative metadata versions for users to upgrade to
  function addUpgradePath(uint256 _tokenId, string calldata _newURI) external onlyOwner {
    bytes32 validityHash = keccak256(abi.encodePacked(_tokenId, _newURI));
    validURI[validityHash] = true;
    emit UpgradePathAdded(_tokenId, _newURI);
  }

  // Owner of token can choose from possible metadata URI's provided by owner of contract
  function upgradeMetadata(uint256 _tokenId, string calldata _newURI) external  {
    require(msg.sender == ownerOf(_tokenId), "DOESNT OWN TOKEN");
    bytes32 validityHash = keccak256(abi.encodePacked(_tokenId, _newURI));
    require(validURI[validityHash], "URI ISNT VALID");
    string memory oldURI = ipfsURI[_tokenId];
    ipfsURI[_tokenId] = _newURI;
    emit MetadataUpgraded(oldURI, _newURI);
  }

  // Returns the full ipfs hash for this token
