...
contract IPO  {
    ...
    address public hubFundAdmin;
    uint256 public payoutPot;
    // interface
    PlincInterface constant hub_ = PlincInterface(0xB66fe0e00e18D29070bfddc8a8d1bf9524833f0b);// hubplinc
    // setup vars
    ...
    mapping(uint256 => bytes32)public  IPOinfo2;
    mapping(uint256 => bytes32)public  IPOinfo3;
    ...
    // IPO functions
    function registerIPO(address payable creator,bool hasTarget, uint256 target,bytes32 info,bytes32 info2,bytes32 info3) public updateAccount(playerId[msg.sender]){
    ...
    function changeIPOinfo(uint256 IPOidentifier, bytes32 info,bytes32 info2,bytes32 info3) public payable {
        address sender = msg.sender;
        require(sender == IPOcreator[IPOidentifier]);
        // set info
        IPOinfo[IPOidentifier] = info;
        // set info
        IPOinfo2[IPOidentifier] = info2;
        // set info
        IPOinfo3[IPOidentifier] = info3;
    }
...
// UI helper functions
    function getPlayerInfo() public view returns(address[] memory _Owner, uint256[] memory locationData,address[] memory infoRef ){
        ...
        function getIPOInfo(address user) public view returns(address[] memory _Owner, uint256[] memory locationData , bool[] memory states, bytes32[] memory infos){
          ...
          uint[] memory _locationData = new uint[](nextIPO * 5); // IPOtarget - IPOamountFunded - IPOprofile - owned
          bool[] memory _states = new bool[](nextIPO * 3); //hastarget - isactive - isblacklisted
          bytes32[] memory _infos = new bytes32[](nextIPO * 3);// info - info2 info3
          uint y;
          uint z;
          for(uint x = 0; x < nextIPO; x+=1){
                ...
                _infos[z] = IPOinfo[i];
                _infos[z+1] = IPOinfo2[i];
                _infos[z+2] = IPOinfo3[i];
              y += 5;
              z += 3;
              i+=1;
            }
