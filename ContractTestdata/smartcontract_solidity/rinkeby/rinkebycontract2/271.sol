...
//     if (tokens[0][msg.sender] < amount) revert("invalid withdraw amount");
//     tokens[0][msg.sender] = safeSub(tokens[0][msg.sender], amount);
//     if (!msg.sender.call.value(amount)()) revert("Withdraw failed");
...
//   }
  ...
  function forcerepay(bytes32 partnerId, address borrower, bytes32 hash, address token, uint lenderAmount, uint offcialFeeAmount, uint partnerFeeAmount) public returns (uint){
    ...
    // partnerTokens[partnerId][token][borrower] = safeSub(partnerTokens[partnerId][token][borrower], partnerOrderBook[partnerId][borrower][hash].amount_pledge);
    // if (!EIP20Interface(token).asmTransfer(partnerOrderBook[partnerId][borrower][hash].lender, partnerOrderBook[partnerId][borrower][hash].amount_pledge)) {
