...
contract GachaDrop is Ownable {
    ...
    bytes32[] public DropNames;
    event _random(address _from, uint _ticket, bytes32 _drop);
    event _changePeriodToPlay(bytes32 _drop, uint _period);
    constructor() public {
        Drops['GodsUnchained'].name = 'GodsUnchained';
        Drops['GodsUnchained'].periodToPlay = 1;
        DropNames.push('GodsUnchained');
    }
    function getDropNames() public view returns(bytes32[] memory) {
        return DropNames;
    }
    function getAward(bytes32 _drop) public {
    ...
    function isValidToPlay(bytes32 _drop) public view returns (bool){
        return Drops[_drop].periodToPlay <= now - Drops[_drop].timeTrackUser[msg.sender];
    }
    function changePeriodToPlay(bytes32 _drop, uint _period) onlyOwner public{
        if(Drops['GodsUnchained'].periodToPlay != 0) DropNames.push(_drop);
