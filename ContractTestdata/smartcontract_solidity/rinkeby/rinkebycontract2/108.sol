...
contract BuyNFT is Ownable {
    ...
    address public ceoAddress = address(0xFce92D4163AA532AA096DE8a3C4fEf9f875Bc55F);
    address public buyNFTProxy = address(0x39f3085825aB27602467b255C0C58e9edE008FeA);
    ...
    constructor() public {
        ...
        //         Games[arrGames[i]].Fee = 10;
        //         Games[arrGames[i]].limitFee = 1000000000000000;
        //         Games[arrGames[i]].limitHBWALLETFee = 1;
        //     } else {
        //         Games[arrGames[i]].Fee = 0;
        //         Games[arrGames[i]].limitFee = 0;
        ...
        Games[address(0xac9D8D6dB95828259069226456DDe98d8E296c5f)].limitFee = 1000000000000000;
    ...
    modifier isProxy() {
        require(buyNFTProxy == msg.sender);
        _;
    }
    ...
    function setPrice(address _game, address payable _sender, uint256 _tokenId, uint256 _price, uint256 _fee, uint _isHightLight) internal {
        Games[_game].tokenPrice[_tokenId] = Price(_sender, _price, _fee, _isHightLight);
    ...
    function calFee(address _game, address _sender, uint256 _price) public view returns (uint256){
        uint256 senderHBBalance = hbwalletToken.balanceOf(_sender);
    ...
    function calPrice(address _game, address _sender, uint256 _tokenId, uint256 _Price, uint _isHightLight) public view
    returns(uint256 _Need) {
        ...
        if (Games[_game].tokenPrice[_tokenId].Price < _Price) {
            fee = calFee(_game, _sender, _Price.sub(Games[_game].tokenPrice[_tokenId].Price));
            totalFee = calFee(_game, _sender, _Price);
    ...
    function setPriceFee(address _game, address payable _sender, uint256 _tokenId, uint256 _Price, uint _isHightLight) public payable isProxy {
        require(Games[_game].tokenPrice[_tokenId].Price != _Price);
        uint256 Need = calPrice(_game, _sender, _tokenId, _Price, _isHightLight);
        ...
        if (Games[_game].tokenPrice[_tokenId].Price < _Price) {
            fee = calFee(_game, _sender, _Price.sub(Games[_game].tokenPrice[_tokenId].Price));
            uint256 totalFee = calFee(_game, _sender, _Price);
        ...
        setPrice(_game, _sender, _tokenId, _Price, fee, _isHightLight);
        emit _setPrice(_game, _tokenId, _Price, _isHightLight, 1);
    }

    function removePrice(address _game, uint256 _tokenId) public isProxy{
    ...
    function setLimitFee(address _game, uint256 _Fee, uint256 _limitFee, uint256 _hightLightFee) public onlyOwner {
    ...
    function changeProxy(address _address) public onlyOwner {
        require(_address != address(0));
        buyNFTProxy = _address;
