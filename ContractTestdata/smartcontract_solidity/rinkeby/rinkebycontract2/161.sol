...
contract Lottery is Croupier, Contracts {
    ...
    // time
    uint32 public disableBuyingTime;
    uint32 public intervalTime;

    // other
    bool public isPause;
    uint64 public nextPrice;
    ...
    // ----------------------- event logs -----------------------
    event LogDraw(uint64 indexed gameIndex, uint startTime, uint bitcoinBlockIndex, bytes numbers, uint riseAmount, uint transferAmount, uint addToJackpotAmount, uint addToReserveAmount);

    event betLog(uint64 indexed gameIndex, address player, uint256 bet, uint8[] numbers, address referer);
    event winLog(uint64 indexed gameIndex, address player, uint256 winnings, uint8[] numbers);


    // ----------------------- constructor function -----------------------
    function Lottery(uint8 _numbersCount) public {
        ...
        // referals[0].push(Referal(msg.sender));
    ...
    function closeCurrentGame(uint bitcoinBlockIndex) public onlyCroupier {
    ...
    function() external payable {
        // bytes betNumbers = getWinNumbers(blockhash(block.number), numbersCount, numbersCountMax);
        // 随机生成一组彩票搭配方案
    ...
    function buyTicket(uint[] numbers, address referer) public payable {
        require(tx.origin == msg.sender, "No contract bet accepted");

        if (referer == msg.sender && referer == address(this)) {
            referer = address(0);
        }

        require(isPause == false, "Game suspended");
        require(numbers.length % numbersCount == 0, "Invalid bet parameters");

        Game storage game = games[gameIndexToBuy];

        uint buyTicketCount = numbers.length / numbersCount;
        require(msg.value == game.price * buyTicketCount, "Invalid bet amount");
        require(game.tickets.length + buyTicketCount <= ticketCountMax, "The maximum buy quantity has been reached");
        ...
        // game.referalIdToTicketCount[referer] += buyTicketCount;

        // emit betLog(_depositId, msg.sender, msg.value, memo);
    }
    ...
    function drawGame(uint bitcoinBlockIndex, string bitcoinBlockHash) public onlyCroupier {
        ...
        uint technicalAmount = riseAmount * technicalPercent / PERCENT_DIVIDER;
        uint dividendsAmount = riseAmount * dividendsPercent / PERCENT_DIVIDER;
        ...
        //                ref.wallet.transfer(riseAmount * ref.percent / PERCENT_DIVIDER);
        //            }
        //        }
    ...
    function calcWins(Game storage game) private {
        ...
        uint riseAmount = game.tickets.length * game.price * (PERCENT_DIVIDER - technicalPercent - dividendsPercent - referalPercent) / PERCENT_DIVIDER;
    ...
    // function getWinNumbers(bytes winNumber) private view returns (uint[]){
    // uint[] memory result = new uint[](numbersCount);
    // bytesToUint(winNumber[0]);
