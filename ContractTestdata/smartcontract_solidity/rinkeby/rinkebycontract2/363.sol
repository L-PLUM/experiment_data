...
contract Distribution is IDistribution, ManagerRole, Ownable, Pausable {
    ...
    function setRewards(address[] calldata accounts, uint256[] calldata values) external whenPaused onlyOwnerOrManager returns (bool) {
        require(accounts.length != 0, "Accounts length is zero");
        require(accounts.length == values.length, "Lengths difference");
    ...
    function collectRewards(address[] calldata accounts) external onlyOwnerOrManager returns (bool) {
        require(accounts.length != 0, "Accounts length is zero");
