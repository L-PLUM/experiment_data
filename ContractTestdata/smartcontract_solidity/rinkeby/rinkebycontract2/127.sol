...
contract vTokenFactory is Ownable {
    address[] public deployedTokens;
    event tokenDeployed(string indexed tokenName, address indexed tokenAddress);
    function createVirtuToken(string memory name, string memory symbol, uint8 decimals,
        address[] memory _owners, uint8 threshold) public onlyOwner returns (address) {
            
            address newToken = address(new vToken(name, symbol, decimals, _owners, threshold));
            
            deployedTokens.push(newToken);
            
            emit tokenDeployed(name, newToken);
    }
...
contract vToken is ERC20, ERC20Detailed, Multiownable {    
