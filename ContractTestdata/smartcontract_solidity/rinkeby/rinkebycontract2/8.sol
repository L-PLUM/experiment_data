...
// UI helper functions
    function getIPOInfo(uint256 IPOidentifier) public view returns(address[] memory _funders, uint256[] memory owned){
        uint i;
          ...
          for(uint x = 0; x < IPOnextAddressPointer[IPOidentifier]; x+=1){
              checkpoint = false;
                for(uint y = 0; y < IPOnextAddressPointer[IPOidentifier]; y+=1){
                    if(IPOadresslist[IPOidentifier][i] ==IPOadresslist[IPOidentifier][y])
                    {
                        checkpoint = true;
                    }
                }
                ...
                _locationData[i] = IPOpurchases[i][IPOadresslist[IPOidentifier][i]];
                }
              i+=1;
            }
