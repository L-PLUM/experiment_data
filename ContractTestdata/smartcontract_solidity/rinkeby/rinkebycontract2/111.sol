...
contract buyNFTInterface {
    ...
    function calPrice(address _game, address _sender, uint256 _tokenId, uint256 _Price, uint _isHightLight) public view returns(uint256 _Need);

    function setPriceFee(address _game, address _sender, uint256 _tokenId, uint256 _Price, uint _isHightLight) public payable;
...
contract BuyNFTProxy is Ownable {
    ...
    constructor() public {
        CryptoCurrencies['ETH'] = address(0xD222D634fBDC893D3442F923ADca752851A00091);
    ...
    function calFee(string memory _crypto, address _game, uint256 _price) public view returns (uint256){
        ...
        return cripto.calFee(_game, msg.sender, _price);
    }
    function calPrice(string memory _crypto, address _game, uint256 _tokenId, uint256 _Price, uint _isHightLight) public view returns(uint256 _Need){
        buyNFTInterface cripto = buyNFTInterface(CryptoCurrencies[_crypto]);
        return cripto.calPrice(_game, msg.sender, _tokenId, _Price, _isHightLight);
    }
    ...
    function setPriceFee(string memory _crypto, address _game, uint256 _tokenId, uint256 _Price, uint _isHightLight) public payable isOwnerOf(_game, _tokenId){
        buyNFTInterface cripto = buyNFTInterface(CryptoCurrencies[_crypto]);
        return cripto.setPriceFee.value(msg.value)(_game, msg.sender, _tokenId, _Price, _isHightLight);
    }
