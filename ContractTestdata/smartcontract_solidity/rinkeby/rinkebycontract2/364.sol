...
contract AxieSushiMath is HasAdmin {
    ...
    returns (uint256 _score, bool[] memory _didCrit, bool[] memory _speedBuffed, uint8[] memory _speedBites)
  {
    ...
    while (_eatenSushis < _sushiCnt) {
      bool _shouldCrit = _getRandomNumber(10000) < _critChance;
      ...
      bool _shouldGainSpeed = _getRandomNumber(10000) < _speedChance;
  ...
  function _getRandomNumber(uint256 _upper) internal returns (uint256) {
...
contract AxieSushi is Pausable, Withdrawable {
  ...
  Race[] public races;
  RaceResult[] public results;
  uint256 public raceLength;
  AxieCore public axieCore;
  AxieSushiMath public axieSushiMath;
  uint256 public maxRacer = 10;
  uint256 public sushiCnt = 20;
  uint256 public operatorReward = 50; // 0.5%
  ...
  function setParameters(uint256 _maxRacer, uint256 _sushiCnt, uint256 _operatorReward) external onlyAdmin {
    maxRacer = _maxRacer;
    sushiCnt = _sushiCnt;
    require(_operatorReward < 10000);
  ...
  function joinRace(
    ...
    require(!_race._finished && _race._racers.length < maxRacer);
    ...
    _race._racers.push(_axieId);
    _race._owners.push(_owner);
  ...
  function startRace(uint256 _raceIndex) external {
    require(_raceIndex < races.length);
    Race storage _race = races[_raceIndex];
    require(_race._racers.length > 1 && !_race._finished);

    uint256 _bestScore = 1 << 255;
    uint256 _winnerIndex = 0;

    for (uint256 i = 0; i < _race._racers.length; i++) {
      uint256 _genes = _race._genes[i];
      ...
      results[_raceIndex] = RaceResult(_score, _didCrit, _speedBuffed, _speedBites);
    ...
    _race._finished = true;
    _race._operator = msg.sender;

    address _winner = _race._owners[_winnerIndex];
    uint256 _reward = _sendReward(_winner, _race);
    emit RaceFinished(_raceIndex, _race._racers[_winnerIndex], _winner, _reward, _race._bettingTokenAddress);
  }
  ...
  function _sendReward(address _winner, Race storage _race) internal returns (uint256 _reward) {
    _reward = _race._bettingValue * (_race._racers.length) * (10000 - operatorReward) / 10000;
