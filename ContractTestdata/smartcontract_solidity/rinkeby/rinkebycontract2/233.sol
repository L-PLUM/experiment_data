...
contract PredictionMarket{
     ...
     struct Insurance
    {
        bytes32 ID;                               // ID (身分證)
        string name;                             // Name
        uint gender;                             // 性別 (0 -> 男性 , 1 -> 女性)
        string insuranceName;                    // 險種名稱
        uint insuranceType;                      // 主約/附約別 (0 -> 主約 , 1 -> 附約)
        uint insuredAmount;                      // 投保保額
        string  companyName;                     // 公司判別
        string itemName;                         // 理賠項目名稱
        uint256 insuranceID;                      // 保單號碼
        uint256 beginTime;                       // 起保日期     
        uint status;                             // 契約狀態  (0 -> 有效 , 1 -> 停效 , 2 -> 無效)
    }
    
    uint public InsuranceCount;
    
    function addInsurance(bytes32 ID
        ,string name 
        ,uint gender
        ...
        ,uint256 insuranceID
        ,uint256 beginTime
        ,uint status) public payable returns(uint EventCount)
        {
            InsuranceCount++;
            emit add_insurance(
                ID,
            ...
            InsuranceList[InsuranceCount]= Insurance({
                ID: ID,
                name: name,
                gender: gender,
  ...
  function uintToBytes(uint v) public pure returns (bytes32 ret) {
    if (v == 0) {
        ...
        while (v > 0) {
            ...
            ret |= bytes32(((v % 10) + 48) * 2 ** (8 * 31)); // 15 or 31   
            v /= 10;
        }
    }
    return ret;
...
//     function uintToBytes(uint x) public pure returns (bytes b) {
//     b = new bytes(16);
