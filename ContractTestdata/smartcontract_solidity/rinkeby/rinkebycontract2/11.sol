...
contract MerkleAirDrop is RootValidator {
	...
	function hash(address sender, uint256 price) public pure returns(bytes32) {
		bytes memory addressBytes = bytes(addressToString(sender));
		bytes memory colon = bytes(":");
		bytes memory priceBytes = bytes(uint2str(price));
		bytes memory part1 = addressBytes.concat(colon);
		
		return keccak256(part1.concat(priceBytes));
	}

	function hash3(address sender, uint256 price) public pure returns(string memory) {
		bytes memory addressBytes = bytes(addressToString(sender));
		bytes memory colon = bytes(":");
		bytes memory priceBytes = bytes(uint2str(price));
		bytes memory part1 = addressBytes.concat(colon);
		
		return string(part1.concat(priceBytes));
	}

	function hash2(string memory test) public view returns(bytes32) {
