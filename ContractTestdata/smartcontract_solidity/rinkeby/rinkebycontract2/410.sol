...
//--> 264 000
    ...
    function checkEnoughBalance (uint256 limitPrice, uint256 tradePrice, uint256 qty, bool side, uint256 fee, uint256 gasFee, bytes32 futuresContractHash, uint256 availableBalance) view returns (bool)
    {
        ...
        uint256 fundingCost = calculateFundingCost(tradePrice, qty, safeSub(expirationBlock, block.number), futuresContractHash);
    ...
    function updateBalances (bytes32 futuresContract, address[2] addressValues, bytes32 positionHash, uint256[8] uintValues, bool[3] boolValues) private
    {
        ...
        */
        ...
        else 
        {
            ...
            pam[3] = safeMul(1e10, calculateFundingCost(retrievePosition(positionHash)[1], uintValues[0], safeSub(block.number, retrievePosition(positionHash)[3]), futuresContract));   
    ...
    function closeFuturesPositionInternal (bytes32 futuresContract, bool side, address user, uint256 gasFee) private
    {
        ...
        {     
            ...
            v.payableFunding = calculateFundingCost(retrievePosition(positionHash)[1], retrievePosition(positionHash)[0], safeSub(block.number, v.entryBlock+1), futuresContract);
    ...
    function closeFuturesContract (bytes32 futuresContract, uint256 price) onlyOracle returns (bool)
    {
        ...
        function closeFuturesContractTemp (bytes32 futuresContract, uint256 price) onlyAdmin returns (bool)
    {
        if (futuresContracts[futuresContract].expirationBlock == 0)  return false; // contract not found
        if (futuresContracts[futuresContract].closed == true)  return false; // contract already closed
