...
contract Gacha is Ownable {
    ...
    function setAward(address _user, address _game, uint256 _tokenId) public {
        IERC721 erc721 = IERC721(_game);
        require(erc721.getApproved(_tokenId) == address(this));
        uint256[] storage tokenIds = awardDatas[msg.sender].items[_game].tokenIds;
        tokenIds.push(_tokenId);
        awardDatas[_user].items[_game].tokenIds = tokenIds;
    ...
    function checkowner(address _game, uint256 _tokenId) public returns(bool) {
        bool valid;
