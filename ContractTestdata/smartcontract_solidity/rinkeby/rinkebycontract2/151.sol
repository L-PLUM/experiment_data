...
contract SpecialFlagging is SpecialFlaggingAccessControl, GameRole {
    ...
    function claimPO8BySkullyWithLocations(uint256 skullyId, uint256[] memory locationsId) public whenNotPaused {
        uint256 locationOwner = allSpecialLocations[allSpecialLocationIndexs[locationsId[0]]].owner;
        uint256 totalPO8;
        for(uint256 i = 0; i < locationsId.length; i++) {
            assert(locationOwner == allSpecialLocations[allSpecialLocationIndexs[locationsId[i]]].owner); //skullyId
        
            totalPO8 += (now - allSpecialLocations[allSpecialLocationIndexs[locationsId[i]]].takenTime) * rate;
        
            allSpecialLocations[allSpecialLocationIndexs[locationsId[i]]].takenTime = now;
        }
