...
contract BuyNFTByETH is Ownable {
    ...
    uint256 public limitHBWALLETForSale = 300; 
    struct Price {
        address payable tokenOwner;
        uint256 ethPrice;
        uint256 ethfee;
        uint isHightlightETH;
    }
    struct Game {
    ...
    constructor() public {
        ...
        //     if(arrGames[i] == address(0x4F41d10F7E67fD16bDe916b4A6DC3Dd101C57394)) {
    ...
    function setPrice(address _game, uint256 _tokenId, uint256 _priceETH, uint256 _ethfee, uint _isHightLightETH) internal {
        Games[_game].tokenPrice[_tokenId] = Price(msg.sender, _priceETH, _ethfee, _isHightLightETH);
    ...
    function calFee(address _game, uint256 _price) public view returns (uint256){
        ...
        uint256 fee =_price.mul(Games[_game].ETHFee).div(Percen);
        if(senderHBBalance >= limitHBWALLETForSale) fee = fee.div(2);
        return fee;
    }
    function calFeeHightLight(address _game, uint256 _tokenId, uint _isHightLight) public view returns (uint256){
        ...
        if (_isHightLight == 1 && (Games[_game].tokenPrice[_tokenId].ethPrice == 0 || Games[_game].tokenPrice[_tokenId].isHightlightETH != 1)) {
            _hightLightFee = hightLightFee;
        }
        return _hightLightFee;
    }
    function calPriceEth(address _game, uint256 _tokenId, uint256 _ethPrice, uint _isHightLight) public view
    returns(uint256 _ethNeed) {
        uint256 ethfee;
        uint256 _hightLightFee = calFeeHightLight(_game, _tokenId, _isHightLight);
        uint256 ethNeed;
        uint256 totalFee;
        if (Games[_game].tokenPrice[_tokenId].ethPrice < _ethPrice) {
            ethfee = calFee(_game, _ethPrice.sub(Games[_game].tokenPrice[_tokenId].ethPrice));
            totalFee = calFee(_game, _ethPrice);
            if(Games[_game].tokenPrice[_tokenId].ethPrice == 0 && ethfee < Games[_game].limitETHFee) {
                ethNeed = Games[_game].limitETHFee.add(_hightLightFee);
            } else if(Games[_game].tokenPrice[_tokenId].ethPrice > 0 && totalFee < Games[_game].limitETHFee) {
                ethNeed = _hightLightFee;
            } else {
                if(totalFee.add(_hightLightFee) < Games[_game].tokenPrice[_tokenId].ethfee) ethNeed = 0;
                else ethNeed = totalFee.add(_hightLightFee).sub(Games[_game].tokenPrice[_tokenId].ethfee);
            }
    ...
    function setPriceFeeEth(address _game, uint256 _tokenId, uint256 _ethPrice, uint _isHightLight) public payable isOwnerOf(_game, _tokenId) {
        require(Games[_game].tokenPrice[_tokenId].ethPrice != _ethPrice);
        uint256 ethNeed = calPriceEth(_game, _tokenId, _ethPrice, _isHightLight);

        require(msg.value >= ethNeed);
        uint256 _hightLightFee = calFeeHightLight(_game, _tokenId, _isHightLight);
        uint256 ethfee;
        if (Games[_game].tokenPrice[_tokenId].ethPrice < _ethPrice) {
            ethfee = calFee(_game, _ethPrice.sub(Games[_game].tokenPrice[_tokenId].ethPrice));
            ...
            if(Games[_game].tokenPrice[_tokenId].ethPrice == 0 && ethfee < Games[_game].limitETHFee) {
                ...
                if(totalFee.add(_hightLightFee) < Games[_game].tokenPrice[_tokenId].ethfee) ethfee = 0;
                else ethfee = totalFee.sub(Games[_game].tokenPrice[_tokenId].ethfee);
            }
            ethfee = ethfee.add(Games[_game].tokenPrice[_tokenId].ethfee);
        } else {

            ethfee = Games[_game].tokenPrice[_tokenId].ethfee;
        }

        setPrice(_game, _tokenId, _ethPrice,ethfee, _isHightLight);
    ...
    function removePrice(address _game, uint256 _tokenId) public isOwnerOf(_game, _tokenId){
        msg.sender.transfer(Games[_game].tokenPrice[_tokenId].ethfee);
    ...
    function setLimitFee(address _game, uint256 _ethFee, uint256 _ethlimitFee, uint256 _hightLightFee) public onlyOwner {
        require(_ethFee >= 0 && _ethlimitFee >= 0 && _hightLightFee >= 0);
    ...
    function cancelBussinessByGameId(address _game, uint256 _tokenId) private {
        ...
        || Games[_game].tokenPrice[_tokenId].tokenOwner == address(this)) {
            ...
            uint256 eth = Games[_game].tokenPrice[_tokenId].ethfee;
            if(Games[_game].tokenPrice[_tokenId].isHightlightETH == 1) eth = eth.add(hightLightFee);
        ...
        for(uint j = 0; j< arrGames.length; j++) {
            ...
            for (uint i = 0; i < Games[_game].tokenIdSale.length; i++) {
                ...
                if (Games[_game].tokenPrice[_tokenId].tokenOwner == erc721Address.ownerOf(_tokenId)) {
                    ...
                    ethfee = ethfee.add(Games[_game].tokenPrice[_tokenId].ethfee);
                    if(Games[_game].tokenPrice[_tokenId].isHightlightETH == 1) ethfee = ethfee.add(hightLightFee);
                }
            }
        }
    ...
    function buy(address _game, uint256 tokenId) public payable {
        ...
        require(Games[_game].tokenPrice[tokenId].ethPrice > 0 && Games[_game].tokenPrice[tokenId].ethPrice == msg.value);
    ...
    function buyFromSmartcontractViaTransfer(address _game, uint256 _tokenId) public payable {
        IERC721 erc721Address = IERC721(_game);
        require(Games[_game].tokenPrice[_tokenId].ethPrice == msg.value);
