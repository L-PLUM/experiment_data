...
contract SpecialFlagging is SpecialFlaggingAccessControl, GameRole {
    ...
    struct Battle {
        ...
        uint256 timeAttack;
        uint256 fee;
        uint8 stage; // 0 : pending, 1 : win, 2 : lose
    }
     ...
     * @return 
     */
    function createBattle(uint256 locationId, uint256 attacker) public whenNotPaused payable {
        ...
        require(checkWinCondition(attacker, locationOwner));
        ...
        emit BattleCreated(id, locationId, attacker, locationOwner, now, fee, 0);
    }
     ...
     * @return 
     */
    function giveUp(uint256 battleId) public whenNotPaused {
        require(isAttackedState[allBattles[battleId].locationId] == true);
        require(allBattles[battleId].stage == 0);
        ...
        isAttackedState[allBattles[battleId].locationId] = false; // set stage of location
        
        allBattles[battleId].stage = 1; // set state win of Battle
     ...
     * @return 
     */
    function defend(uint256 battleId) public whenNotPaused {
        address payable defenderOwner = address(uint160(address(nonFungibleContract.ownerOf(allBattles[battleId].defender))));
        require(msg.sender == defenderOwner);
        ...
        require(checkWinCondition(allBattles[battleId].defender, allBattles[battleId].attacker));
        ...
        allBattles[battleId].stage = 2; // set state win of Battle
     ...
     * @return 
     */
    function checkWinCondition(uint256 skully1, uint256 skully2) public view returns (bool) {
        uint8 skully1Rank;
        (, skully1Rank) = skItem.getSkullyFlagRank(skully1);
        uint8 skully2Rank;
        (, skully2Rank) = skItem.getSkullyFlagRank(skully2);
        require(skully1Rank > 0 && skully2Rank > 0);
        if(skully1Rank > skully2Rank) // because rank 1 > rank 2
            return false;
        
        uint256 skully1AccessoriesPower;
        skully1AccessoriesPower = skItem.getTotalAccessoriesAP(skully1);
        uint256 skully2AccessoriesPower;
        skully2AccessoriesPower = skItem.getTotalAccessoriesDP(skully2);
        
        // calculate attackerPower
        uint256 skully1AttackPower;
        (skully1AttackPower,,,,,) = nonFungibleContract.getSkully(skully1);
        
        uint256 totalSkully1AttackPower = skully1AccessoriesPower + skully1AttackPower;
        
        // calculate defenderPower
        uint256 skully2DefendPower;
        (,skully2DefendPower,,,,) = nonFungibleContract.getSkully(skully2);
        
        uint256 totalSkully2DefendPower = skully2AccessoriesPower + skully2DefendPower;
        
        if(totalSkully1AttackPower < totalSkully2DefendPower)
            return false;
