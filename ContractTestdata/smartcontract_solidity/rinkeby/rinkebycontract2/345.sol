...
contract PlayerWonCoin is BurnableToken {
    ...
    */
    ...
    //Star of the crowdsale
     uint256 startTime;
     uint256 phaze1Start = 1564351200; 
    ...
    uint256 rate1 = 3767600000000000000000; 
      ...
      */
      ...
      constructor(address _wallet, BurnableToken _token, uint256 starttime) public{
     ...
     function setCrowdsale(address _wallet, BurnableToken _token, uint256 starttime) public onlyOwner{
     ...
     function buyTokens(address _beneficiary) public whenNotPaused payable {
       ...
       if ((block.timestamp >= phaze1Start ) && (block.timestamp <= phaze1End)) {
         rate = rate1;
       }
       else if ((block.timestamp >= phaze2Start) && (block.timestamp <= phaze2End)) {
        rate = rate2;
       }
       else if ((block.timestamp >= phaze3Start) && (block.timestamp <= phaze3End)) {
        rate = rate3; 
       }
       else {
...
uint256 tmp = rate.div(1000000000000000000); 
   ...
   contract WonCrowdsale is Crowdsale {
    ...
    using SafeMath for uint256;
    constructor(address wallet, BurnableToken token, uint256 startTime, uint256 cap) Crowdsale(wallet, token, starttime) public onlyOwner
    {
