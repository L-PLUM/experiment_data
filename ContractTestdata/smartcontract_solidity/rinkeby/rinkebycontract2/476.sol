...
contract LocalEthereum_1 {
    ...
    function disputeByMediator(uint16 _tradeId, address payable _seller, address payable _buyer, uint256 _amount, uint16 _favour, uint16 _type, address _tokenContract) external onlyMediator returns(bool)
    {
   ...
   // Withdraw fees collected by the contract. Only the owner can call this.
    ...
    function disputeCall(uint16 _tradeId, address payable _seller, address payable _buyer, uint256 _amount, uint16 disputetype) private returns (bool)
    {
        bytes32 _tradeHash = keccak256(abi.encodePacked(_tradeId,_seller,_buyer,_amount));
