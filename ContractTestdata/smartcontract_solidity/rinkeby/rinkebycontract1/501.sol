...
contract splitProfit is FDDCoin {
    ...
    function sendProfit(address _personAddress)public{
        ...
        uint calc3;
        time =now *1 hours;
        delatTime = time - DEPOSIT[counter][_personAddress].Time;
        for(uint i=0;i<IDs.length;i++){
            sum+= DEPOSIT[counter][_personAddress].Amount * delatTime;
        }
        for(uint k=0;k<depositorAddress.length;k++){
            Sum+=DEPOSIT[counter][depositorAddress[k]].Amount * delatTime;
        }
         calc1= (sum/Sum);
         calc2=calc1*profit;
