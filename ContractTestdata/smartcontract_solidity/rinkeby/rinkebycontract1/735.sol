...
contract FadAlbaniaChain {
    ...
    modifier checkParam(string memory inputFadId, string memory inputEvent) { 
        require(bytes(inputFadId).length>0 && bytes(inputEvent).length>0);
        emit ReportError(inputFadId, "Parametri non valorizzati");
        _; 
    ...
    function ActWriteStep(uint256 inputCourseId, uint256 inputUserId, string memory inputFadId, 
                          ...
                          public checkAdmin() checkParam(inputFadId, inputEvent) returns (bool)
    {
