...
contract iABToken is AccessControl{
    ...
    function setMaxAngels() public;
    function setMaxAccessories() public;
    function setMaxMedals()  external ;
    function initAngelPrices() public;
    function initAccessoryPrices()public ;
    function setCardSeriesPrice(uint8 _cardSeriesId, uint _newPrice) public;
    function approve(address to, uint256 tokenId) public;
    function getRandomNumber(uint16 maxRandom, uint8 min, address privateAddress) view public returns(uint8) ;
    ...
    function createOther(uint16 _seriesID) external ;
    ...
    function getABToken(uint256 tokenId) view public returns(uint8 cardSeriesId, uint16 power, uint16 auraRed, uint16 auraYellow, uint16 auraBlue, string memory name, uint16 experience, uint64 lastBattleTime, uint16 lastBattleResult, address owner);
    ...
    function getABTokenByIndex(address _owner, uint64 _index) view public returns(uint256) ;
...
contract ABToken is IERC721, iABToken, ERC165 {
    ...
    uint32[73] public currentTokenNumbers;
    uint32[73] public maxTokenNumbers;
...
function setMaxAngels() public onlyCREATOR {
    uint i =0;
...
//Initial function to set the max number of medals
  function setMaxMedals() onlyCREATOR external  {
    ...
    function initAngelPrices() public onlyCREATOR {
       angelPrice[0] = 0;
    ...
    function initAccessoryPrices() public onlyCREATOR {
    ...
    function setCardSeriesPrice(uint8 _cardSeriesId, uint _newPrice) public onlyCREATOR {
   ...
   */
    ...
    */
    ...
    function mintABToken(address owner, uint8 _cardSeriesId, uint16 _power, uint16 _auraRed, uint16 _auraYellow, uint16 _auraBlue, string memory _name, uint16 _experience, uint16 _oldId) public onlySERAPHIM {
        require(currentTokenNumbers[_cardSeriesId] < maxTokenNumbers[_cardSeriesId]);
    ...
    function buyAngel(uint8 _angelSeriesId) public payable {
        ...
        if (maxTokenNumbers[_angelSeriesId] <= currentTokenNumbers[_angelSeriesId]) {revert();}
        ...
        currentTokenNumbers[_angelSeriesId] ++;
    ...
    function buyAccessory(uint8 _accessorySeriesId) public payable {
        ...
        //don't create another card if they haven't sent enough money. 
        if (msg.value < accessoryPrice[_accessorySeriesId]) {revert();} 
        ...
        currentTokenNumbers[_accessorySeriesId] ++;
    ...
    function createOther(uint16 _seriesID) external {}
    //dev function to create cards not specified here to increase game functionality later on
    //devs cannot create any current card series. 
