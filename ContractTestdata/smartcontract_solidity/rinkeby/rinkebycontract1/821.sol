...
contract Race{
  ...
  function enterRace(uint axieId) public payable{
    ...
    require(_genes!=0);//axie exists and is adult
  ...
  */
  ...
  function processRacer(uint axieId,uint randomSeed) public returns(uint timeToFinish){
      ...
      emit RacerParams(axieId,raceId,uintValues[4],uintValues[5],uintValues[6]);
      ...
      while (uintValues[1] < TOTAL_SUSHIS) {
        ...
        uintValues[7]++;
        emit RaceStep(axieId,raceId,didCrit,gainSpeed,uintValues[1]);
        if(didCrit){
          uintValues[1]+=2;
        }
        else{
      ...
      emit FinalScore(axieId,raceId,uintValues[0]);
      return uintValues[0];
  }
