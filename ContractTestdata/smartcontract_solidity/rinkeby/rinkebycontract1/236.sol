...
contract DOSProxy {
    ...
    event LogPublicKeyUploaded(uint groupId, uint[4] pubKey, uint count, uint groupSize, bool equal);
        ...
        onlyWhitelisted
    {
        ...
        for (uint i = 0; i < pendingGroup.length; i++) {
            if (pendingGroup[i].groupId == groupId) {
                ...
                emit LogPublicKeyUploaded(groupId, pubKey, count, groupSize, BN256.G2Equal(pendingGroup[i].firstPubKey, newPubKey));
