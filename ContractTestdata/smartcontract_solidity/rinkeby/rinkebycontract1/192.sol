...
contract LandSecterAsset is ERC721Full, ERC721Mintable, ERC721Pausable {
    ...
    mapping(uint16 => mapping(uint16 => uint256)) private landTypeAndRarityToLandSecterCount;
    ...
    function isValidLandSecter(uint256 _tokenId) public view returns (bool) {
        uint16 rarity = getRarity(_tokenId);
        if (!(rarityToSecterVolume[rarity] > 0)) {
            return false;
        }
        ...
        if (serial > landTypeAndRarityToSecterSupply[landType][rarity]) {
            return false;
        }
        return true;
    }
    ...
    function setSupplyAndSecter(
        ...
        require(getMintedSecterCount(_landType) == 0, "This LandType already exists");
        require(
    ...
    function getMintedSecterCount(uint16 _landType) public view returns (uint256) {
        return landTypeToLandSecterIndex[_landType];
    }

    function getMintedSecterCountByRarity(uint16 _landType, uint16 _rarity) public view returns (uint256) {
        return landTypeAndRarityToLandSecterCount[_landType][_rarity];
    }
    ...
    function getMintedSecterList(uint16 _landType) public view returns (uint256[] memory) {
    ...
    function mintLandSecter(address _owner, uint256 _landSecter) public onlyMinter {
        ...
        require(isValidLandSecter(_landSecter));
        ...
        require(landTypeAndRarityToLandSecterCount[_landType][rarity] < landTypeAndRarityToSecterSupply[_landType][rarity], "supply over");
        _mint(_owner, _landSecter);
        landTypeToLandSecterList[_landType].push(_landSecter);
        landTypeToLandSecterIndex[_landType]++;
        landTypeAndRarityToLandSecterCount[_landType][rarity]++;
    }
...
contract MCHLandPool is Ownable, Pausable, ReentrancyGuard {
	...
	function withdrawMyReward(uint256 _landSecter) external whenNotPaused() nonReentrant() {
	...
	function getLandSecterWithdrawableBalance(uint256 _landSecter) public view returns (uint256) {
		require(landSecterAsset.isValidLandSecter(_landSecter));
