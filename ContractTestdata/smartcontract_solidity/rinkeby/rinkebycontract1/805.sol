...
contract EmcoTokenOld is StandardToken, Ownable {
...
contract Clan is Ownable {
    ...
    mapping(address => mapping(uint => bool)) reclaimedRewards;
    ...
    constructor(address _clanOwner, address _emcoToken, uint256 _epochLength) public {
        ...
        epochLength = _epochLength; //todo set from constructor argument
        emco = EmcoToken(_emcoToken);
    }

    function replenish(uint amount) public onlyOwner {
        uint currentEpoch = getCurrentEpoch();
        epochRewards[currentEpoch] = epochRewards[currentEpoch].add(amount);
    ...
    function getMembersForEpoch(uint epochNumber) view public returns (uint membersNumber) {
    ...
    function reclaimReward(uint256 epoch) public {
        uint currentEpoch = getCurrentEpoch();
        require(currentEpoch > epoch, "you can request reward only for previous epochs");
        require(epochJoined[msg.sender] != 0, "User not is not a member of clan for this epoch");
        require(reclaimedRewards[msg.sender][epoch] == false, 
            "you can reclaim reward only once per epoch");
        uint epochReward = epochRewards[epoch];
        require(epochReward > 0, "no reward for this epoch");

        uint ownersReward = epochReward.div(10);
        uint userReward = (epochReward - ownersReward).div(membersNumForEpoch[epoch]);

        if(msg.sender == clanOwner) {
            userReward = userReward.add(ownersReward);
        } 

        require(emco.transfer(msg.sender, userReward), "token should be transferred");
...
contract EmcoToken is StandardToken, Ownable {
    ...
    function migrate(uint amount) public {
        require(oldContract.transferFrom(msg.sender, this, amount));
    ...
    function referralCodeOwners(bytes32 _code) public view returns (address owner) {
        address oldContractOwner = oldContract.referralCodeOwners(_code);
        if(oldContractOwner == address(0)) {
            return refCodeOwners[_code];
        } else {
            return oldContractOwner;
        }
    }
    ...
    function setReferral(bytes32 _code) public {
        ...
        require(oldContract.referrals(msg.sender) == address(0), "Ref is set");
	...
	*/
    function createClan(uint256 epochLength) public returns (address clanAddress) {
        ...
        require(userClans[msg.sender] == address(0x0), "clan exists");
		//check if user has at least 10 invitees
        require(inviteeCount[msg.sender] >= 10, "min 10 invitees");
