...
contract TreasuryRight is IManaged, ERC20{
    using Address for address;
    address private _estate;
    mapping(uint=>uint) lastMint;
    function TreasuryRight(string memory name, string memory symbol, uint8 decimals) public 
    ...
    function mint(uint tokenId) public returns (uint minted){
        minted = canMint(tokenId);
        require(minted > 0);
        lastMint[tokenId] = now;
        _mint(IEstate(_estate).getAgent(tokenId), minted);
    }
    ...
    function canMint(uint tokenId) public view returns (uint avail){
        uint lastMintDate = lastMint[tokenId];
        uint elapsed = now - lastMintDate;
        avail = lastMintDate == 0 ? 42 : elapsed / 90 days;
        avail = avail > 42 ? 42 : avail;
        return avail;
    }
