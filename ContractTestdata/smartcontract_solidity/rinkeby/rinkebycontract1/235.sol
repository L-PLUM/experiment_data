...
contract DOSProxy {
    ...
    event LogPublicKeyUploaded(uint groupId, uint[4] pubKey, uint count, uint groupSize, bool equal, uint groupIdx);
        ...
        onlyWhitelisted
    {
        ...
        for (uint i = 0; i < pendingGroup.length; i++) {
            if (pendingGroup[i].groupId == groupId) {
                if (!pendingGroup[i].set) {
                    pendingGroup[i].firstPubKey = newPubKey;
                    pendingGroup[i].set = true;
                }
                pendingGroup[i].pubKeys[msg.sender] = newPubKey;
                uint count = 0;
                for (uint j = 0; j < pendingGroup[i].adds.length; j++) {
                    if (BN256.G2Equal(pendingGroup[i].pubKeys[pendingGroup[i].adds[j]], newPubKey)) count = count + 1;
                }
                emit LogPublicKeyUploaded(groupId, pubKey, count, groupSize, BN256.G2Equal(pendingGroup[i].firstPubKey, newPubKey), i);
...
//                    pendingGroup.length -= 1;
    ...
    function grouping(address[] memory candidates, uint size) public onlyWhitelisted {
        ...
        while (candidates.length >= index + groupSize) {
            ...
            BN256.G2Point memory firstPubKey;
            uint groupId = uint(keccak256(abi.encodePacked(
                    ++requestIdSeed, lastRandomness, toBeGrouped[lastRandomness % groupSize])));
            pendingGroup.push(Group({groupId: groupId, adds: toBeGrouped, finPubKey: finPubKey, birthBlkN: block.number, set: false, firstPubKey: firstPubKey}));
