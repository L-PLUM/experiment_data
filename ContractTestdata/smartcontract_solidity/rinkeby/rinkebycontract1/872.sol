...
// File: contracts/Email.sol

contract Email is SnowflakeResolver {
    struct User{
        string email;
    }
    ...
    constructor (address snowflakeAddress)
        SnowflakeResolver("Email", "Set your Email.", snowflakeAddress, true, false) public
    {}
    ...
    function getEmail(uint ein) public view returns (string memory) {
        return user[ein].email;
    }

    function setEmail(string memory email) public returns(bool success){
        SnowflakeInterface snowflake = SnowflakeInterface(snowflakeAddress);
        ...
        //if(bytes(user[ein].email).length == 0){
        user[ein].email = email;
        emit EmailUpdated(ein, email);
        //}
        return true;
    }
    ...
    event EmailUpdated(uint ein, string email);
