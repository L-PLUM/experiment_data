...
contract Operator {
    ...
    uint256 public MAX_DEP = 30 ether;
    ...
    address payable public operationFund = 0x7316E599cAd568CA05C37F09bA1891A705b2BD10;
    ...
    function pushNewMaxOut(address investor, uint256 times) internal {
        bytes32 id = keccak256(abi.encodePacked(block.number, now, investor, times));
        MaxOut memory maxOut = MaxOut({ id: id, at: now, investor: investor, times: times });
        maxOutIds.push(id);
        maxOuts[id] = maxOut;
    }
    ...
    function sendEtherForInvestor(address payable investorAddress, uint256 value, uint256 reason, address presentee, uint256 times) internal {
        ...
        if (withdrewAmount + value >= 3 * depositedAmount) {
            ...
            pushNewMaxOut(investorAddress, investor.maxOutTimes);
        } else {
