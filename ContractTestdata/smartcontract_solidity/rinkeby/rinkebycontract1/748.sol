...
contract HoQuAdCampaign is HoQuAdCampaignI, HoQuStorageAccessor {
    ...
    function test3(bytes16 trackerId, uint256 price) public {
        rater.processAddLead(offerId, trackerId, affiliateId, price);
    }

    function test4(bytes16 id, bytes16 trackerId, bytes16 tariffId, string meta, string dataUrl, uint256 price, HoQuStorageSchema.Status _status) public {
        leads[id] = Lead({
            createdAt : now,
            trackerId : trackerId,
            tariffId : tariffId,
    ...
    function test5(bytes16 id, uint256 price) public {
