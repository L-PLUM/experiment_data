...
contract LandSecterAsset is ERC721Full, ERC721Mintable, ERC721Pausable {
    ...
    mapping(uint16 => uint256) private landTypeToToTalSecter;
    mapping(uint16 => mapping(uint16 => uint256)) private secterSupplyByLandTypeAndRarity;
    ...
    function setSupplyAndSecter(
        ...
        require(_totalSecter != 0, "totalSecter must not be 0");
        ...
        landTypeToToTalSecter[_landType] = _totalSecter;
        secterSupplyByLandTypeAndRarity[_landType][LEGENDARY_RARITY] = legendarySupply;
        secterSupplyByLandTypeAndRarity[_landType][EPIC_RARITY] = epicSupply;
        secterSupplyByLandTypeAndRarity[_landType][RARE_RARITY] = rareSupply;
        secterSupplyByLandTypeAndRarity[_landType][UNCOMMON_RARITY] = uncommonSupply;
        secterSupplyByLandTypeAndRarity[_landType][COMMON_RARITY] = commonSupply;
    }
    ...
    function getTotalSecter(uint16 _landType) public view returns (uint256) {
        return landTypeToToTalSecter[_landType];
    }
    ...
    function getShareRateWithDecimal(uint256 _landSecter) public view returns (uint256, uint256) {
		return (getSecterVolumeByRarity(getRarity(_landSecter))
            ...
            .div(getTotalSecter(getLandType(_landSecter))),
            SHARE_RATE_DECIMAL);
    }
    ...
    function mintLandSecter(address _owner, uint256 _landSecter) public onlyMinter {
        ...
        require(getMintedSecterAmount(_landType) <= secterSupplyByLandTypeAndRarity[_landType][getRarity(_landSecter)], "supply over");
...
contract MCHLandPool is Ownable, Pausable, ReentrancyGuard {
	...
	function addEthToLandPool(uint16 _landType) external whenNotPaused() nonReentrant() payable {
		require(landSecterAsset.getTotalSecter(_landType) > 0);
	...
	function withdrawMyAllRewards() external whenNotPaused() nonReentrant() {
		...
		uint256 withdrawValue = getWithdrawableBalance(msg.sender);
		
		for (uint256 i=balance; i > 0; i--) {
			uint256 landSecter = landSecterAsset.tokenOfOwnerByIndex(msg.sender, i-1);
			landSecterToWithdrawnAmount[landSecter] += getLandSecterWithdrawableBalance(landSecter);
		}
...
contract MCHGUMGatewayV6 is DJTBase {
	...
	constructor(
		address _landSecterAssetAddress,
	...
	function buyGUM(uint16 _landType, address payable _referrer, bytes calldata _signature) external payable whenNotPaused() nonReentrant() {
		...
		if (landSecterAsset.getTotalSecter(_landType) != 0) {
			landValue = msg.value.mul(lordEthBackRate).div(100);
			netValue = netValue.sub(landValue);
			landPool.addEthToLandPool.value(landValue)(_landType);
		}
