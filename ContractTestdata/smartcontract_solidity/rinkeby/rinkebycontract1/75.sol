...
contract Note is ERC721, ERC721Enumerable, ERC721Metadata, MinterRole {

    event NoteCreated(address owner, uint256 tokenId);

    ERC20   public feeToken;
    address public treasury;
    uint256 public fee;

    struct OriginalTerms {
    ...
    mapping(uint256 => OriginalTerms) private originalTerms;

    constructor(
        string name,
        string symbol,
        address treasuryAddress,
        uint256 defaultFee) public ERC721Metadata(name, symbol) {
        treasury = treasuryAddress;
        fee = defaultFee;
    }

    function setFeeToken(address noteBlockToken) public onlyMinter {
        feeToken = ERC20(noteBlockToken);
    }

    function updateFee(uint256 newFee) public onlyMinter {
        require(fee != newFee);
        fee = newFee;
    }
    ...
    function mintWithDetails(
        ...
        //first take the fee from the owner, before minting
        require(feeToken.transferFrom(to, treasury, fee));

        _mint(to, tokenId);
        _setTokenURI(tokenId, tokenURI);
        originalTerms[tokenId] = OriginalTerms(
            propertyState,
            propertyType,
            loanAmount,
            firstPaymentDate,
    ...
    function getNoteDetails(uint256 tokenId) public view returns (
        ...
        return (
            originalTerms[tokenId].propertyState,
            originalTerms[tokenId].propertyType,
            originalTerms[tokenId].loanAmount,
            originalTerms[tokenId].firstPaymentDate,
            originalTerms[tokenId].interestRate,
            originalTerms[tokenId].owner
        );
    }

    function getNotesOf(address holder) public view returns (uint256[]) {
...
contract NoteBlockToken is MultiOwnable, ERC20 {
    using SafeMath for uint256;

    event Transfer(address _from, address _to, uint256 _value);
    event Approval(address _owner, address _spender, uint256 _value);

    string public constant name = "NoteBlock Token";
     ...
     */
    constructor(Note noteAddress) public {
        note = noteAddress;
    ...
    */
    ...
    function createNote(
        ...
        increaseApproval(address(note), note.fee());

        return note.mintWithDetails(
                msg.sender,
                tokenId,
                tokenURI,
                propertyState,
