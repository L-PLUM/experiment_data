...
// File: contracts/LandSecterAsset.sol
...
contract LandSecterAsset is ERC721Full, ERC721Mintable, ERC721Pausable {
    ...
    mapping(uint16 => uint256) private landTypeToSecterSupplyLimit;
    mapping(uint16 => mapping(uint16 => uint256)) private landTypeAndRarityToSecterSupply;
    mapping(uint16 => uint256[]) private landTypeToLandSecterList;
    mapping(uint16 => uint256) private landTypeToLandSecterIndex;
    mapping(uint16 => mapping(uint16 => uint256)) private landTypeAndRarityToLandSecterIndex;
	mapping(uint16 => uint256) private rarityToSecterVolume;

    constructor() public ERC721Full("MyCryptoHeroes:Land", "MCHL") {
		rarityToSecterVolume[5] = 100;
		rarityToSecterVolume[4] = 20;
		rarityToSecterVolume[3] = 5;
		rarityToSecterVolume[2] = 2;
		rarityToSecterVolume[1] = 1;
    }
    ...
    function setSupplyAndSecter(
        uint16 _landType,
        uint256 _totalVolume,
        uint256 _secterSupplyLimit,
        ...
        require(getMintedSecterAmount(_landType) == 0, "This LandType already exists");
        require(
            legendarySupply.mul(rarityToSecterVolume[LEGENDARY_RARITY])
            .add(epicSupply.mul(rarityToSecterVolume[EPIC_RARITY]))
            .add(rareSupply.mul(rarityToSecterVolume[RARE_RARITY]))
            .add(uncommonSupply.mul(rarityToSecterVolume[UNCOMMON_RARITY]))
            .add(commonSupply.mul(rarityToSecterVolume[COMMON_RARITY]))
            == _totalVolume
        );
        require(
            legendarySupply
        ...
        landTypeToSecterSupplyLimit[_landType] = _secterSupplyLimit;
        landTypeAndRarityToSecterSupply[_landType][LEGENDARY_RARITY] = legendarySupply;
        landTypeAndRarityToSecterSupply[_landType][EPIC_RARITY] = epicSupply;
        landTypeAndRarityToSecterSupply[_landType][RARE_RARITY] = rareSupply;
        landTypeAndRarityToSecterSupply[_landType][UNCOMMON_RARITY] = uncommonSupply;
        landTypeAndRarityToSecterSupply[_landType][COMMON_RARITY] = commonSupply;
    }
    ...
    function getSecterSupplyLimit(uint16 _landType) public view returns (uint256) {
        return landTypeToSecterSupplyLimit[_landType];
    }

    function getLandType(uint256 _landSecter) public view returns (uint16) {
        uint16 _landType = uint16((_landSecter.div(10000)) % 1000);
        return _landType;
    }

    function getRarity(uint256 _landSecter) public view returns (uint16) {
        return uint16(_landSecter.div(10**7));
    }

    function getMintedSecterAmount(uint16 _landType) public view returns (uint256) {
        return landTypeToLandSecterIndex[_landType];
    }

    function getMintedSecterAmountByRarity(uint16 _landType, uint16 _rarity) public view returns (uint256) {
        return landTypeAndRarityToLandSecterIndex[_landType][_rarity];
    }

    function getSecterSupplyByRarity(uint16 _landType, uint16 _rarity) public view returns (uint256) {
        return landTypeAndRarityToSecterSupply[_landType][_rarity];
    }

    function getSecterList(uint16 _landType) public view returns (uint256[] memory) {
        return landTypeToLandSecterList[_landType];
    }

	function getSecterVolumeByRarity(uint16 _rarity) public view returns (uint256) {
		return rarityToSecterVolume[_rarity];
	}

    function getShareRateWithDecimal(uint256 _landSecter) public view returns (uint256, uint256) {
		return (getSecterVolumeByRarity(getRarity(_landSecter))
            .mul(SHARE_RATE_DECIMAL)
            .div(getTotalVolume(getLandType(_landSecter))),
            SHARE_RATE_DECIMAL);
    }

    function mintLandSecter(address _owner, uint256 _landSecter) public onlyMinter {
        require(!isAlreadyMinted(_landSecter));
        uint16 _landType = getLandType(_landSecter);
        require(landTypeToLandSecterIndex[_landType] <= landTypeToSecterSupplyLimit[_landType]);
        uint16 rarity = getRarity(_landSecter);
        require(landTypeAndRarityToLandSecterIndex[_landType][rarity] <= landTypeAndRarityToSecterSupply[_landType][rarity], "supply over");
        _mint(_owner, _landSecter);
        landTypeToLandSecterList[_landType].push(_landSecter);
        landTypeToLandSecterIndex[_landType]++;
        landTypeAndRarityToLandSecterIndex[_landType][rarity]++;
    }
...
/* solhint-enable no-mix-tabs-and-spaces*/
...
contract MCHLandPool is Ownable, Pausable, ReentrancyGuard {
	...
	LandSecterAsset public landSecterAsset;
	address public validater;
	
	mapping(uint16 => uint256) private landTypeToTotalAmount;
	mapping(uint256 => uint256) private landSecterToWithdrawnAmount;
	...
	event WithdrawEther(
		uint256 indexed landSecter,
		address indexed lord,
		uint256 value,
		uint256 at
	);

	constructor(address _landSecterAssetAddress) public {
	    landSecterAsset = LandSecterAsset(_landSecterAssetAddress);
	}

	function updateLandSecterAssetAddress(address _landSecterAssetAddress) external onlyOwner() {
		landSecterAsset = LandSecterAsset(_landSecterAssetAddress);
	}
	...
	function addEthToLandPool(uint16 _landType, address _purchaseBy) external whenNotPaused() nonReentrant() payable {
		require(landSecterAsset.getTotalVolume(_landType) > 0);
	...
	function withdrawMyAllRewards() external whenNotPaused() nonReentrant() {
		uint256 balance = landSecterAsset.balanceOf(msg.sender);
		require(balance > 0);

		uint256 withdrawValue;
		
		for (uint256 i=balance; i > 0; i--) {
			uint256 landSecter = landSecterAsset.tokenOfOwnerByIndex(msg.sender, i-1);
			uint256 tmpAmount = getLandSecterWithdrawableBalance(landSecter);
			withdrawValue += tmpAmount;
			landSecterToWithdrawnAmount[landSecter] += tmpAmount;
	...
	function withdrawMyReward(uint256 _landSecter) public whenNotPaused() nonReentrant() {
		require(landSecterAsset.ownerOf(_landSecter) == msg.sender);
		uint256 withdrawableAmount = getLandSecterWithdrawableBalance(_landSecter);
		landSecterToWithdrawnAmount[_landSecter] += withdrawableAmount;
	...
	function getLandSecterWithdrawnAmount(uint256 _landSecter) public view returns (uint256) {
		return landSecterToWithdrawnAmount[_landSecter];
	}

	function getLandSecterWithdrawableBalance(uint256 _landSecter) public view returns (uint256) {
		uint16 _landType = landSecterAsset.getLandType(_landSecter);
		require(landSecterAsset.getTotalVolume(_landType) > 0);
		(uint256 shareRate, uint256 decimal) = landSecterAsset.getShareRateWithDecimal(_landSecter);
		uint256 maxAmount = landTypeToTotalAmount[_landType]
			.mul(shareRate)
			.div(decimal);
		return maxAmount.sub(landSecterToWithdrawnAmount[_landSecter]);
	}

	function getWithdrawableBalance(address _lordAddress) public view returns (uint256) {
		uint256 balance = landSecterAsset.balanceOf(_lordAddress);
		uint256 withdrawableAmount;

		for (uint256 i=balance; i > 0; i--) {
			uint256 landSecter = landSecterAsset.tokenOfOwnerByIndex(_lordAddress, i-1);
			withdrawableAmount += getLandSecterWithdrawableBalance(landSecter);
		}
...
contract MCHGUMGatewayV6 is DJTBase {
	...
	uint256 public lordEthBackRate;
	...
	event lordEthBackRateUpdated(
		uint256 lordEthBackRate
	);
	...
	function setLandSecterAssetAddress(address _landSecterAssetAddress) external onlyOwner() {
        landSecterAsset = LandSecterAsset(_landSecterAssetAddress);
    }
	...
	function updateLordEthBackRate(uint256 _newLordEthBackRate) external onlyOwner() {
		lordEthBackRate = _newLordEthBackRate;
		emit lordEthBackRateUpdated(
			lordEthBackRate
		);
	}
	...
	// function buyGUM(uint16 _landType, address payable _referrer) external payable whenNotPaused() nonReentrant() {
	function buyGUM(uint16 _landType, address payable _referrer, bytes calldata _signature) external payable whenNotPaused() nonReentrant() {
		...
		require(validateSig(_signature, bytes32(uint256(msg.sender) + uint256(_landType))), "invalid signature");
		...
		if (landSecterAsset.getTotalVolume(_landType) != 0) {
			landValue = msg.value.mul(lordEthBackRate).div(100);
