...
contract Estate is IManaged, ERC721Full {
    ...
    //Only a manager can mint for free
    function mint(address addr) public payable managed unlocked returns (uint){
        _mint(addr,lastID++);
        return lastID;
    }

    //Only the trust can authorize / initiate a token burn
    function burn(uint tokenID) public payable managed unlocked returns (bool){
        _burn(tokenID);
        return true;
    }
    ...
    function setELF(address elf) public locked trustonly{
        _elf = elf;
    }

    //Can be used by the ELF contract to sieze Estates for non-payment of debt and sell them to the highest bidder
    function surrendered(uint tokenId) public unlocked{
        require(msg.sender == _elf);
        address from = ownerOf(tokenId);
        _transferFrom(from, _elf, tokenId);
    }

    function transfer(uint tokenId, address to) public unlocked{
        require(msg.sender == ownerOf(tokenId) || msg.sender == _elf);
        address from = ownerOf(tokenId);
        _transferFrom(from, to, tokenId);
    }
}
