...
contract DOSProxy {
    ...
    struct Group {
        uint groupId;
        address[] adds;
        mapping(uint => uint8) pubKeyCounts;
        ...
        onlyWhitelisted
    {
        ...
        for (uint i = 0; i < pendingGroup.length; i++) {
            if (pendingGroup[i].groupId == groupId) {
                pendingGroup[i].pubKeyCounts[pubKey[0]] = pendingGroup[i].pubKeyCounts[pubKey[0]] + 1;
                emit LogPublicKeyUploaded(groupId, pubKey, pendingGroup[i].pubKeyCounts[pubKey[0]], groupSize);
                if (pendingGroup[i].pubKeyCounts[pubKey[0]] > groupSize / 2) {
