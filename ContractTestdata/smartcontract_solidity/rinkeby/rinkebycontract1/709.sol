...
contract FadAlbaniaChain {
                          ...
                          public checkAdmin()  returns (bool)
    {
        ...
        require(((keccak256(bytes(eventTemp.Event)) != keccak256("ModulePassed") && keccak256(bytes(eventTemp.Event)) != keccak256("ModuleFailed") && Courses[inputCourseId][inputUserId][eventTemp.Event].length == 0) 
        ...
        //Passa se non sei in fase CoursePassed/Failed o hai almeno un evento di tipo ModulePassed
        require(Courses[inputCourseId][inputUserId]["ModulePassed"].length > 0  
        ...
        require((keccak256(bytes(eventTemp.Event)) != keccak256("Enrollment") && Courses[inputCourseId][inputUserId]["Enrollment"].length == 1)
        ...
        require(Courses[inputCourseId][inputUserId]["CoursePassed"].length > 0  
        ...
        require(keccak256(bytes(eventTemp.Event)) == keccak256("Enrollment") 
             ||(Courses[inputCourseId][inputUserId]["Enrollment"][0].DateStart <= eventTemp.Date && Courses[inputCourseId][inputUserId]["Enrollment"][0].DateEnd >= eventTemp.Date), "Data operazione non compresa tra data di inizio e fine corso");
