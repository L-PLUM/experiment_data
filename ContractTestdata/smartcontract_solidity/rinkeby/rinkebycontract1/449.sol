...
contract TumblerGame is TumblerGameInterface,TumblerEvents,Ownable{
   ...
   function assertInvestPercentage(
        ...
        //平台直接分配+代理商直接分配+奖池+分红池+推荐人直接分配 不能超过90%
        require(_platformDistributePercent.add(_adminDistributePercent).add(_gainPoolDistributePercent).add(_profitPoolDistributePercent).add(_totalLayerDistributePercents)<=90,"设置每个主体的投资分配超过了90%");
   ...
   //TODO:从分红池提取收益
    ...
    function withdrawGain() external{
        ...
        uint256 userAmount=userTotalGain[msg.sender];
