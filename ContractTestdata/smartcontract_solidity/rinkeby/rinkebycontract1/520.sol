...
contract LakeDiamond is Ownable {
    ...
    */
    function createOrder(uint typeId, string metadata, uint price, uint tknAmount, address[] bannedAddresses) external payable {
        require(msg.value > 0);
    ...
    */
    function addAddressToWhitelist(address userAddress, uint allowedTokenAmount) external {
    ...
    */
    function createProposal(uint price, uint amount) external {
    ...
    */
    function setOrderType(uint id, string description) external {
    ...
    */
    function matchOrder(uint orderId, uint[] ids) public {
        ...
        uint orderCoeficient = orderPrice.mul(orderTokenAmount).div(orderStorageAdapter.getEthAmount(orderId));
        ...
        for (uint i = 0; i < ids.length; i++) {
            ...
            orderTokenAmount = orderTokenAmount.sub(proposalTokenAmount);
            getToken().burn(proposalTokenAmount);

            uint proposalEtherAmount = proposalTokenAmount.mul(proposalStorageAdapter.getPrice(proposalId)).div(orderCoeficient);
    ...
    */
    ...
    function getOrderStorageAdapter() internal view returns (OrderStorageAdapter orderStorageAdapter) {
        orderStorageAdapter = OrderStorageAdapter(lakeDiamondStorage.orderStorageAdapterAddress());
    }

    function getProposalStorageAdapter() internal view returns (ProposalStorageAdapter proposalStorageAdapter) {
        proposalStorageAdapter = ProposalStorageAdapter(lakeDiamondStorage.proposalStorageAdapterAddress());
    }

    function getToken() internal view returns (IBurnableERC20 token) {
        token = IBurnableERC20(lakeDiamondStorage.tokenContract());
    }
    
    function getWhitelistKey(address userAddress) internal view returns (bytes32 key) {
        key = keccak256(abi.encode(whiteListKey, userAddress));
    }

    function getSubOwnerKey(address subOwnerAddress) internal view returns (bytes32) {
