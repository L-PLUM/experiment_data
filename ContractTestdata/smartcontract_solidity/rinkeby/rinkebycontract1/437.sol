...
contract PodminersToken is Ownable, ERC223 {
    using SafeMath for uint256;
    // Token properties
    string public name = "Podminers Token";
    ...
    // Constructor
    // @notice PodminersToken Contract
    ...
    function transfer(address _to, uint _value, bytes memory _data) public {
        ...
        require(CheckTransferable(msg.sender));
    ...
    function transferFrom(address from, address to, uint256 value) public canTradable isActive {
        ...
        require(CheckTransferable(from));
    ...
    function CheckTransferable(address from) internal view returns(bool) {
        if(role[from] == 2&&now >= projsealDate)
        {
            return false; 
        }
        if((role[from] == 3 || role[from] == 4)&&now >= partnersealDate)
        {
            return false;
        }
        return true;      
