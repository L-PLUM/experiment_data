...
contract TumblerGame is TumblerGameInterface,TumblerEvents,Ownable{
    ...
    //记录玩家奖金提现 地址=>余额
    mapping(address=>uint256) userTotalGainBalance;
    //记录玩家见点奖励 地址=>见点总额
   ...
   function assertInvestPercentage(
    ...
    function investCore(uint256 amount) internal
    {
        ...
        addGain(msg.sender, remainAmount);
        // userTotalGain[msg.sender]=userTotalGain[msg.sender].add(remainAmount);
        // userTotalGainBalance[msg.sender]=userTotalGainBalance[msg.sender].add(remainAmount);
    ...
    function addGain(address user, uint256 amount) internal {
        userTotalGain[user]=userTotalGain[user].add(amount);
        userTotalGainBalance[user]=userTotalGainBalance[user].add(amount);
        userRoundGain[user][nowRoundId]=userRoundGain[user][nowRoundId].add(amount);
    }
    ...
    function distributeInvestAmount(uint256 amount,Round storage currentRound) internal returns(uint256){
        ...
        addGain(platformAddr, platFormDistributeAmount);
        // userTotalGain[platformAddr]=userTotalGain[platformAddr].add(platFormDistributeAmount);
        // userTotalGainBalance[platformAddr]=userTotalGainBalance[platformAddr].add(platFormDistributeAmount);
        ...
        addGain(admin, adminDistributeAmount);
        ...
        for(uint256 i=senderInviteUserIds.length-1;i>=0;i--){
            ...
            addGain(partnerAddress, partnerDistributeAmount);
    ...
    function distributionGain(Round storage currentRound) internal returns(uint256 remainAmount){
        ...
        addGain(platformAddr, platFormGain);
        ...
        for(uint256 i=winnerInviteUserIds.length-1;i>=0;i--){
            ...
            addGain(partnerAddress, partnerGain);
   ...
   //TODO:从分红池提取收益
    ...
    function withdrawGain() external{
        ...
        uint256 userAmount=userTotalGainBalance[msg.sender];
        require(userAmount>0, "非分红提现金额不足");
        // 转账
        if (tokenType==1) {     // eth
            msg.sender.transfer(userAmount);
        } else if (tokenType==2) {      // 代币
            require(token.transfer(msg.sender, userAmount),"奖池可提现额度不足");
        } else {
            require(false,"合约货币类型错误");
        }
        // 提现后相关记录和修改
        userTotalGainBalance[msg.sender]=0;
        ...
        )
    {
        ...
        uGainBalance=userTotalGainBalance[user];
    ...
    function queryProfitAmount(address user, uint256 roundId) external view returns(uint256 userAmount) {
        ...
        // 提现金额计算
        if(round.keys>0) userAmount=uesrKeys.mul(round.profitPoolAmount).div(round.keys);
        else userAmount=0;
    }
}
