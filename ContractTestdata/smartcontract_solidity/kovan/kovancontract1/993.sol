...
contract GroupedManager is MultiSign, SignerSetter, Erc20Setter, ShouldMultiSign, ImplementationSlector {
    ...
    function _setMethodGroup(bytes32 group) internal {
        if (group == PAUSE_GROUP) {
            ...
            // "issuerTransfer(address,address,uint256)"
            _setGroup(0x935175cf, group);
            
        } else if (group == LAWENFORCEMENT_GROUP) {
            // "wipeBlacklistedAddress(address,address)"
            _setGroup(0x0daaab59, group);
            
            // "wipeBlacklistedIssuer(address,address)"
            _setGroup(0x6f6565fc, group);
        }
    }
    ...
    function _removeMethodGroup(bytes32 group) internal {
        if (group == PAUSE_GROUP) {
            ...
            // "issuerTransfer(address,address,uint256)"
            _setGroup(0x935175cf, INVALID_GROUP);

        } else if (group == LAWENFORCEMENT_GROUP) {
            // "wipeBlacklistedAddress(address,address)"
            _setGroup(0x0daaab59, INVALID_GROUP);
            
            // "wipeBlacklistedIssuer(address,address)"
            _setGroup(0x6f6565fc, INVALID_GROUP);
