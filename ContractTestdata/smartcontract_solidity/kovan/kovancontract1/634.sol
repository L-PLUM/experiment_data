...
contract SplitMeUp {
    ...
    // Struct for storing Private Key Holder Details in addition 
    // to whether they have paid or not
    struct PKHolder{
        string username;
        bool paid;
        uint reward;
    ...
    // mapping of Private Key Holders against there address
    mapping(address => PKHolder) privateKeyHolders;
    // mapping to check if username already used or not
    mapping(bytes32 => address) privateKeyUsernameMap;
    ...
    function addStorageAccountByAdmin(string _username, address _storageAddress) public{
        require(msg.sender == owner);
        address holder = _storageAddress;
        keyStorageAccounts[convert(_username)] = holder;
        numberOfKeyStores++;
    }
    
    function storageUsernameValidity(string _username) public view returns(bool){
        if(keyStorageAccounts[convert(_username)] != 0){
            return true;
        }else{
            return false;
        }
    ...
    function privateKeyUsernameValidity(string _username) public view returns(bool){
        if(privateKeyUsernameMap[convert(_username)] != 0){
            return true;
        }else {
            return false;
        }
    ...
    function addPrivateKeyHolder(string _username) public{
        ...
        privateKeyUsernameMap[convert(_username)] = currAddress;
