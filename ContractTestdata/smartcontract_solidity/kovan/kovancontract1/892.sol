...
contract ArbitrableTokenList is PermissionInterface, Arbitrable {
    ...
    /** @dev Return the numbers of tokens in the list per status. This function is O(n) at worst, where n is the number of tokens. This could exceed the gas limit, therefore this function should only be used for interface display and not by other contracts. This function is different from `tokenCount()` since it counts tokens with disputed pending requests twice.
