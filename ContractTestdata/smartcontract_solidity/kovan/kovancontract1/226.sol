...
contract BBPack is Ownable, ERC721Token {
    ...
    event PackMinted(uint id, address to, uint typeId);
    ...
    constructor(address _BBArtefactAddress, address _manger, address _wallet) public ERC721Token("BBPack Token", "BBPT") {
        wallet = _wallet;
    ...
    function buyPack(uint packTypeId, uint artefactsCount) public payable returns (uint) {
        ...
        packs[incrementPackId] = Pack(incrementPackId, packTypeId, artefactsCount);
        emit PackBought(incrementPackId, msg.sender, packTypeId, msg.value, artefactsCount);
    ...
    function mint(address to, uint typeId, uint artefactsCount) public onlyOwnerOrManager returns (uint) {
        ...
        packs[incrementPackId] = Pack(incrementPackId, typeId, artefactsCount);
        emit PackMinted(incrementPackId, msg.sender, typeId);
        return incrementPackId;
    }
    ...
    function generateLimited(uint packId, PackType packType) internal {
        ...
        for (i = 0; i < packs[packId].artefactsCount; i++) {
            ...
            uint index = getRandomArtefactIndex(packType, random);
            artefactsLeft--;
    ...
    function getRandomArtefactIndex(PackType packType, uint random) internal view returns (uint){
        for (uint j = 0; j < packType.artefactsLeft.length; j++) {
            uint counter = 0;
            if (random < counter + packType.artefactsLeft[j]) {
                return j;
            }
            counter += packType.artefactsLeft[j];
        }
    }
