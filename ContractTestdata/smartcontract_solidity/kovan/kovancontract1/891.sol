...
/// @notice Initiates the atomic swap.
    ...
    function initiateWithFees(
        ...
        // ERC20(TOKEN_ADDRESS).approve(address(this), _value)
        // before this contract can make transfers on the initiator's behalf.
        CompatibleERC20(TOKEN_ADDRESS).transferFrom(msg.sender, address(this), _value + _brokerFee);
    ...
    function audit(bytes32 _swapID) external view returns (uint256 timelock, uint256 value, address to, address from, bytes32 secretLock) {
        return (
            swaps[_swapID].timelock,
            swaps[_swapID].value,
            swaps[_swapID].spender,
            swaps[_swapID].funder,
            swaps[_swapID].secretLock
        );
    }
