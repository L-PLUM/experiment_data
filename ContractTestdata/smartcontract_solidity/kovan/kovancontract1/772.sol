...
contract VNETTokenPreSale is Ownable {
    ...
    uint256 public ethPrice; // with 18 decimals
    ...
    function calcRatioNext() private {
        a = 12345;
        b = ethPrice.mul(10 ** 6);
        c = vnetPriceStart.add(vnetPriceTarget.sub(vnetPriceStart).mul(vnetSold.div(vnetSupply)));
        d = vnetPriceTarget.sub(vnetPriceStart).mul(vnetSold.div(vnetSupply));
        e = vnetPriceTarget.sub(vnetPriceStart).mul(vnetSold).div(vnetSupply);
