...
/** @title ChainInfo
 * @dev This is ChainInfo
 */

contract ChainInfo is Ownable {
    
    using SafeMath for uint256;
    uint256 private _blockNumberAtGenesis;
    uint private _blockTimestampAtGenesis;
    
    constructor () public {
        _blockNumberAtGenesis = block.number;
        _blockTimestampAtGenesis = block.timestamp;
    }
    
    // NOTE  Write Get details function complete
    /**
     * @dev chainInfoDetails
     * @return blockNumberAtGenesis
     * @return blockNumberToken

     * @return blockTimestamp
     */
    function chainInfoDetails() external view
    returns (uint256 blockNumberAtGenesis, uint256 blockNumberToken, uint blockTimestamp, uint256 tokenTimestampAge) {
        blockNumberAtGenesis = _blockNumberAtGenesis;
        blockNumberToken = blockNumberOfToken();
        blockTimestamp = block.timestamp;
        tokenTimestampAge = tokenTimestamp();
    }

    // REVIEW No one uses this function
    /**
     * @dev blockNumber
     * @return uint 
    */  
    function blockNumber() internal view returns (uint256 blocknumber) {
     ...
     * @return uint 
    */  
    function blockTimestamp() internal view returns (uint timestamp) {
        timestamp = block.timestamp;
    }

    /**
     * @dev blockNumberOfToken: Age of token, measured in blocks
     * @return uint256
     */
    function blockNumberOfToken() internal view returns (uint256 blocknumber) {
        blocknumber = block.number.sub(_blockNumberAtGenesis);
    }
    
    /**
     * @dev tokenTimestamp: Age of token, measured in blocks
     * @return uint256
     */
    function tokenTimestamp() internal view returns (uint256 timestamp) {
        timestamp = block.timestamp.sub(_blockTimestampAtGenesis);
    }
...
contract ProofOfTrident is ERC20Detailed, ChainInfo, Remote {
    
    event SentToStake(address from, address to, uint256 value);  

    // Allow max total supply to be set.
    uint256 private _maxTotalSupply = 100000000000 * 10**uint(18);
    // DONE Allow minimum coins to be set
    // minimum number of tokens that can be staked
    uint256 internal _stakeMinimumInTokens = 100000000 * 10**uint(18); 
    // maximum number of tokens that can be staked
    uint256 internal _stakeMaximumInTokens = 500000000 * 10**uint(18); 
    // DONE Allow the minimum number of days staking to be set
    //uint internal _stakeMinimumDays = 0; // minimum age for coin age: 1Days
    ...
    // DONE Allow this to be set
    // DONE Stop the maximum stake at this to encourage users to collect regularly. 
    
    //uint internal _stakeMaximumDays = 365; // stake age of full weight: 30Days
   
    uint internal _stakeMaximumTimestamp = _oneYear + _sixMonths;
    // During Beta, coins will generated more. 
    // Users will expect to see something. Will be edited later through public admin field.
    ...
    // DONE Allow this to be set, be sure to make the amount for the user recalculate
    ...
    function cancelTrident() external returns (bool) {
        ...
        _transferIns[msg.sender].blockNumberOfToken = blockNumberOfToken();
        _transferIns[msg.sender].tokenTimestamp = tokenTimestamp();
    ...
    */
    function setTridentDetails(
        ...
        external onlyOwner returns (bool success) {
            _stakeMinimumInTokens = stakeMinimumInTokens * 10**uint(18);
            _stakeMaximumInTokens = stakeMaximumInTokens * 10**uint(18);
    ...
    /**
    @dev tridentDetails
    @return stakeMinimumInTokens The token count minimum
    @return stakeMaximumInTokens The token count minimum
    ...
    */
    ...
    uint256 maxTotalSupply) {
        ...
        stakeMinimumInTokens = _stakeMinimumInTokens;
        stakeMaximumInTokens = _stakeMaximumInTokens;     
    ...
    function minimumNumberOfTokens () external view returns (uint256) {
        return _stakeMinimumInTokens;
    }

    function maximumNumberOfTokens () external view returns (uint256) {
        return _stakeMaximumInTokens;
    }
    ...
    function myBlockNumberOfToken() external view returns (uint) {
        return  _transferIns[msg.sender].blockNumberOfToken;
    }

    function myTokenTimestamp() external view returns (uint) {
        return _transferIns[msg.sender].tokenTimestamp;
    }   
    ...
    function tridentReward(address owner) public view returns (uint256 totalReward) {
        ...
        // £100 * (1 second) * 2 percentage / 100 / seconds in a year
    ...
    /**
     ...
     @param owner The owner address
     @return holdAgeTimestamp The stake age in days
     */
    function _holdAgeTimestamp(address owner) internal view returns (uint256 holdAgeTimestamp) {
        ...
        uint256 _lengthOfHoldTimestamp = (tokenTimestamp() - _transferIns[owner].tokenTimestamp);
...
contract Vault is ProofOfTrident, ERC20Burnable {
    ...
    uint256 private constant INITIAL_SUPPLY = 0 * (10 ** uint256(DECIMALS));
    ...
    */
    function vaultRequestFromUser () external canPoSMint returns (string memory message) {
        ...
        require(amountAllowed <= _stakeMaximumInTokens, "The allowance has been set too high.");
        ...
        require(amountAllowed >= _stakeMinimumInTokens,
    ...
    * @return uint percentageOfTokensReturned,
