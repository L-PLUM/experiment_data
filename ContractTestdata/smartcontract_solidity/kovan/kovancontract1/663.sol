...
contract rebalance {
    ...
    address payable public oasis_=0xdB3b642eBc6Ff85A3AB335CFf9af2954F9215994; //kovan
...
//    uint256 public constant timePassive_ = 12 weeks;//3 month
    uint256 public timePassive_ = 1 minutes;//for debug
...
//        uint ethedgeBalance=ethedgeToken_.calculateEthereumReceived(ethedgeSupply);
        uint ethedgeBalance=address(ethedge_).balance;
...
//        uint totalDAIbalanceAtETHforBuy=totalDAIbalance.mul(pricePrecision_).div(bestSellPrice);
    ...
    function sellETHbuyDAI(uint _amount) private {
        ...
        uint profit=0;
        if (SellPrice>lastBuyPrice_){ //we have profits, calculate and send back
            profit=_amount.mul(SellPrice).div(lastBuyPrice_);
            distributeProfit(profit);
        }
        lastBuyPrice_=SellPrice;
...
//        require(msg.sender.call.value(wethAmt)());
        ...
        uint profit=0;
        if (BuyPrice<lastSellPrice_){ //we have profits, calculate and send back
            profit=wethAmt.mul(lastSellPrice_).div(BuyPrice);
            distributeProfit(profit);
        }
        lastSellPrice_=BuyPrice;
    ...
    function checkRescue() public {
        ...
        ethedgeToken_.payDividends.value(address(this).balance)('Rescue trade bot ETH action!');
        //try transfer all DAI to dev address
        uint daiAmt=daiToken_.balanceOf(address(this));
        if (daiAmt>0){
            if (daiToken_.allowance(address(this), oasis_) < daiAmt) {
                daiToken_.approve(oasis_, uint(-1));
            }
            //if Oasis was broken this call do nothing and we can try get back DAI to dev
            //oasisDex_.sellAllAmount(dai_, daiAmt, weth_,0);
            oasis_.call(abi.encodeWithSignature("sellAllAmount(address,uint256,address,uint256)",dai_,daiAmt,weth_,0));
            //oasis_.call(bytes4(keccak256("sellAllAmount(address,uint256,address,uint256)")),dai_,daiAmt,weth_,0);
            uint wethAmt=wethToken_.balanceOf(address(this));
            wethToken_.withdraw(wethAmt);
            ethedgeToken_.payDividends.value(wethAmt)('Rescue trade bot DAI action!');
        }
        //if oasisDex broken - try send back all DAI to dev address
        daiAmt=daiToken_.balanceOf(address(this));
        if (daiAmt>0){
    ...
    //If more than 1*1e5 - sell ETH events can be done
    ...
    //If more than 1*1e5 - buy ETH events can be done
