...
contract MakerDaoGateway is Pausable, DSMath {
    ...
    // specify cdpId if you want to use existing CDP, or pass 0 if you need to create a new one 
    function supplyWethAndBorrowDai(bytes32 cdpId, uint wethAmount, uint daiAmount) external {
    ...
    function repayDaiAndReturnEth(bytes32 cdpId, uint daiAmount, uint ethAmount, bool payFeeInDai) external {
    ...
    function repayDaiAndReturnWeth(bytes32 cdpId, uint daiAmount, uint wethAmount, bool payFeeInDai) external {
        repayDai(cdpId, daiAmount, payFeeInDai);
        returnWeth(cdpId, wethAmount);
    }

    // don't forget to approve DAI before repaying
    function repayDai(bytes32 cdpId, uint daiAmount, bool payFeeInDai) public {
        require(cdpOwner[cdpId] == msg.sender, "CDP belongs to a different address");
        if (daiAmount > 0) {
