...
contract CutieGenerator is CutiePluginBase
{
    ...
    function setupPlugin(address _operator, uint40 _momId, uint40 _dadId, uint _limit)
        public
        onlyOwner
    {
    ...
    function addOperator(address _newOperator)
    public
    onlyOwner
    {
    ...
    function removeOperator(address _oldOperator)
    public
    onlyOwner
    {
        delete operator[_oldOperator];
    }


    function generate(address[] _target) public
    {
        require(operator[msg.sender]);
        generated += _target.length;
        require(generated <= limit);

        uint fee = coreContract.getBreedingFee(momId, dadId);
        ...
        for (uint i = 0; i < _target.length; i++)
        {
            ...
            uint40 babyId = coreContract.breedWith.value(fee)(momId, dadId);

            coreContract.changeCooldownIndex(babyId, 0);
            coreContract.changeGeneration(babyId, 0);

            coreContract.transfer(_target[i], babyId);
        }
    }

    function recoverCutie(uint40 _cutieId) public
    {
        require(operator[msg.sender]);
