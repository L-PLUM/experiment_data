...
contract GroupedErc20 is BlacklistManage {
    ...
    event IssuedTransfer(address indexed fromIssuer, address indexed toIssuer, uint256 amount, string proposal);
    ...
    event BlacklistedAddressWiped(address indexed issuer, address indexed addr, uint256 value, string proposal);
    event BlacklistedIssuerWiped(address indexed redIssuer, address indexed badIssuer, uint256 balance, uint256 issuedAmount, string proposal);
    ...
    // lawEnforcement
    function wipeBlacklistedAddress(address issuer, address _account, string proposal) 
        whenNotPaused 
    ...
    returns (bool)  
    {
        ...
        emit BlacklistedAddressWiped(issuer, _account, _balanceFrom, proposal);
    ...
    function doWipeBlacklistedIssuer (address badIssuer, address redIssuer, string proposal) internal returns (bool) {
        ...
        emit BlacklistedIssuerWiped(redIssuer, badIssuer, _balanceFrom, _issuedFrom, proposal);
        
        emit IssuedTransfer(badIssuer, redIssuer, _issuedFrom, proposal);
    ...
    function wipeBlacklistedIssuer(address badIssuer, address redIssuer, string proposal) 
        whenNotPaused 
    ...
    returns (bool)  
    {
        return doWipeBlacklistedIssuer(badIssuer, redIssuer, proposal);
    }
    ...
    function doIssuerTransfer(address _from, address _to, uint256 _value) internal returns (bool) {
    ...
    function issuerTransfer(address _from, address _to, uint256 _value, string proposal) 
        whenNotPaused 
        onlyIssuer(_from) 
        onlyIssuer(_to) 
        notBlacklisted(_from) 
    ...
    returns (bool result) 
    {
        result = doIssuerTransfer(_from, _to, _value);
        emit IssuedTransfer(_from, _to, _value, proposal);
    }
