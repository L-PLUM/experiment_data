...
contract EverbloomExchange is Ownable, ReentrancyGuard, LibMath {
    ...
    // whitelists[whitelistType][address][allowed]
    ...
    function setWhitelist(WhitelistType wlType, address addr, bool allowed) external onlyOwner {
    ...
    returns (Common.OrderInfo memory orderInfo)
    {
        ...
        if (
            !whitelists[uint8(WhitelistType.RESELLER)][order.orderAddresses[4]] ||
            !whitelists[uint8(WhitelistType.VERIFIER)][order.orderAddresses[6]] ||
            !whitelists[uint8(WhitelistType.BANK)][order.orderAddresses[7]] ||
            !whitelists[uint8(WhitelistType.BANK)][order.orderAddresses[8]]
        ) {
    ...
    view
    {
        ...
        if (orderInfo.filledTakerAmount == 0) {
            ...
            if (order.orderValues[4] > 0) {
                require(takerAmountToFill >= order.orderValues[4], "ORDER_MINIMUM_NOT_REACHED");
            }
        }
    ...
    returns (Common.FillResults memory results)
    {
        ...
        if (!whitelists[uint8(WhitelistType.FEE_EXEMPT_BANK)][order.orderAddresses[7]]) {
        ...
        if (!whitelists[uint8(WhitelistType.FEE_EXEMPT_BANK)][order.orderAddresses[8]]) {
    ...
    returns (Common.MatchedFillResults memory results)
    {
        ...
        if (!whitelists[uint8(WhitelistType.FEE_EXEMPT_BANK)][leftOrder.orderAddresses[7]]) {
        ...
        if (!whitelists[uint8(WhitelistType.FEE_EXEMPT_BANK)][rightOrder.orderAddresses[7]]) {
