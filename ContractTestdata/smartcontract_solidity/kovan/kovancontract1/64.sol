...
contract ERC20Standard is ERC20 {
  ...
  function transferFrom(address _from, address _to, uint256 _value) public returns (bool) {
   ...
   function burn(uint256 _value) public onlyOwner {
        require(_value <= balances[msg.sender], "token balances insufficient");
  ...
  function decreaseApproval(address _spender, uint _subtractedValue) public returns (bool) {
   ...
   function () payable{
      ...
      //amount = (msg.value.mul(_tokenBuyPrice) / 1 ether) * 10 ** uint256(_decimals);
    //   uint256 amount = (msg.value) / (_tokenBuyPrice) * 10 ** uint256(_decimals);
...
//       require(msg.value > 0, "Không đủ eth");
//       //amount = (msg.value.mul(_tokenBuyPrice) / 1 ether) * 10 ** uint256(_decimals);
//       amount = (msg.value / _tokenBuyPrice) * 10 ** uint256(_decimals);
//       if(amount > balances[owner]){
//           revert();
//           return 0;
//       }
