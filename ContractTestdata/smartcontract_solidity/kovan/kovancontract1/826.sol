...
contract ContractReceiver {
    function tokenFallback(address _from, uint _value, uint _number) {
...
contract BloToken {
    /* Contract Constants */
    string public constant _name = "BloToken";
    string public constant _symbol = "BLT";
    uint8 public constant _decimals = 8;

    /* The supply is initially 100,000,000BLT to the precision of 8 decimals */
    uint256 public constant _initialSupply = 8000000000000000;
    ...
    event Transfer(address indexed from, address indexed to, uint value, uint number);

    /* Non-ERC Events */
    event Burn(address indexed from, uint256 amount, uint256 currentSupply, bytes data);

    /* ERC20 Functions */
    /* Return current supply in smallest denomination (1BLT = 100000000) */
    ...
    /* Transfer the balance from the sender's address to the address _to with data _data */
    function transfer(address _to, uint _value, uint _number) returns (bool success) {
        if (balances[msg.sender] >= _value
            && _value > 0
            && balances[_to] + _value > balances[_to]) {
            if(isContract(_to)) {
                return transferToContract(_to, _value, _number);
            } else {
                return transferToAddress(_to, _value, _number);
            }
        } else {
            return false;
        }
    ...
    /* Transfer function when _to represents a regular address */
    function transferToAddress(address _to, uint _value, uint _number) internal returns (bool success) {
        balances[msg.sender] -= _value;
        balances[_to] += _value;
        Transfer(msg.sender, _to, _value);
        Transfer(msg.sender, _to, _value, _number);
        return true;
    }

    /* Transfer function when _to represents a contract address, with the caveat
    that the contract needs to implement the tokenFallback function in order to receive tokens */
    function transferToContract(address _to, uint _value, uint _number) internal returns (bool success) {
        balances[msg.sender] -= _value;
        balances[_to] += _value;
        ContractReceiver receiver = ContractReceiver(_to);
        receiver.tokenFallback(msg.sender, _value, _number);
        Transfer(msg.sender, _to, _value);
        Transfer(msg.sender, _to, _value, _number);
        return true;
    }
