...
contract Game is Ownable {
    ...
    event NewBet(uint roundID, address player);
    event ConfirmBet(uint roundID, address player);
    ...
    function setBet(bytes32 _betHash) public returns (uint) {
        require(rounds[numRounds].numPlayers < 2, "Max players num is 2");
        ...
        emit NewBet(numRounds, msg.sender);
    ...
    function confirmBet(uint256 _roundID, uint256 _playerNum, uint256 _hand, uint256 _secretKey) public {
        ...
        emit ConfirmBet(_roundID, msg.sender);
    }
    
    function getWinner(uint256 _roundID) public view returns (address) {
        require(rounds[_roundID].players[0].confirm, "Player 1 is not confirm bet");
        require(rounds[_roundID].players[1].confirm, "Player 2 is not confirm bet");
