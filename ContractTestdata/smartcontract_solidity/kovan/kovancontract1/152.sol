...
contract DealsSupport is IDealsSupport {
  ...
  event ApproveAffiliate(bytes4 _dealID, address indexed _merchantAddress, uint indexed _dealIndex, address indexed _affiliateAddress);
  event CreateDeal(bytes4 _dealID, address indexed _merchantAddress, address indexed _rewardTokenAddress, uint _rewardRatePpm, uint _daysOfCancellation, uint _daysBeforeClose, uint _dealsCount);
  event BlockAffiliate(bytes4 _dealID, uint _referenceHash);
  event DealCancelled(bytes4 _dealID, uint _days, address _who);
  event StakeFilled(bytes4 _dealID, uint _fill, uint _left);
  event RewardClaimed(bytes4 _dealID, uint _referenceHash);
  ...
  */
  //Should
  function dealIndexToDealdealID(uint _dealIndex) public view returns(bytes4) {
  ...
  */
  function blockAffiliate(uint _referenceHash, bool _isBlocked) public onlyAdmin validReferenceHash(_referenceHash) {
    ...
    emit BlockAffiliate(dealID, _referenceHash);
  }
  ...
  */
  function getMyReward(uint _dealIndex) public validDealIndex(_dealIndex) {
    ...
    emit RewardClaimed(deals[_dealIndex].dealID, referenceHash);
  }
  ...
  */
  function approveAffiliate(uint _dealIndex, address _affiliateAddress) public validDealIndex(_dealIndex) activeDeal(_dealIndex) {
    if (!deals[_dealIndex].allowAdminToAddAffiliate) {
      ...
      require((msg.sender == adminAddress) || (msg.sender == merchantAddress));
    }
    // validation
    require(_affiliateAddress != address(0));
    ...
    // emitting event
    emit ApproveAffiliate(deals[_dealIndex].dealID, msg.sender, _dealIndex, _affiliateAddress);
  }
  ...
  */
  ...
  function createDeal(bytes4 _dealID, address _rewardTokenAddress, uint _rewardRatePpm, uint _daysOfCancellation, uint _daysBeforeClose, bool _allowAdminToAddAffiliate) public onlyMerchant {
    ...
    emit CreateDeal(_dealID, msg.sender, _rewardTokenAddress, _rewardRatePpm, _daysOfCancellation, _daysBeforeClose, dealsCount - 1);
  }
...
contract MultistageCrowdsale is DealsSupport, Ownable {
  ...
  function invest(bytes32 _r, bytes32 _s, bytes32 _a, bytes32 _b) public payable {
    ...
    require(outOfBandPaymentAmount+invested[beneficiary]<=getMaxInvAt(now) || getMaxInvAt(now)==0,"Investment over maximum allowed");
    ...
    require(purchasedTokenAmount > 0);
  ...
  function storeStages(uint256[] _timesAndRates) internal {
    ...
    require(_timesAndRates.length % 4 == 1);
    // check that at least 1 stage provided
    require(_timesAndRates.length >= 5);

    for (uint256 i = 0; i < _timesAndRates.length / 4; i++) {
        require(uint64(_timesAndRates[(i * 4) + 3]) == 0 ||
            uint64(_timesAndRates[(i * 4) + 2]) <= uint64(_timesAndRates[(i * 4) + 3]),
            "Maximum should be higher than minimum, or 0");
        stages.push(Stage(uint32(_timesAndRates[i * 4]), uint64(_timesAndRates[(i * 4) + 1]),
            uint64(_timesAndRates[(i * 4) + 2]), uint64(_timesAndRates[(i * 4) + 3])));
      if (i > 0) {
