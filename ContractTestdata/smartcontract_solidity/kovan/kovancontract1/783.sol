...
contract ClinicAllCrowdsale is Crowdsale, FinalizableCrowdsale, MintedCrowdsale, ManagedWhitelist, Limited {
  ...
  (
    ...
    uint256 _privateSaleSupplyLimit,
    ERC20 _token,
  ...
  *  Properties, constants
  ...
  // Private sale tokens minted
  bool public privateSaleMinted = false;
  ...
  */
  ...
  {
    return (preSaleClosingTime >= block.timestamp && token.totalSupply() < discountTokenAmount.add(privateSaleSupplyLimit));
  }
  ...
  */
  function allocatePrivateSaleFunds() public onlyOwner
  {
    require(!privateSaleMinted, "Token's for private sale minted only once.");
    require(privateSaleSupplyLimit < CappedToken(token).cap(), "Token's private sale supply limit should be less that token' cap limit.");
    _deliverTokens(privateSaleWallet, privateSaleSupplyLimit);
    privateSaleMinted = true;
  }
}
