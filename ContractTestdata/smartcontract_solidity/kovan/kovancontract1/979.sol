...
contract CryptoHeart is ERC721, Ownable {
  ...
  function bulkMint(address _to, uint256 _amount) public onlyOwner returns (uint256 lowestId) {
    uint256 nextId;
    lowestId = 0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff;
  ...
  function _random(uint256 seed) private view returns (uint256) {
    return uint256(keccak256(abi.encodePacked(seed, block.timestamp, block.difficulty)));
  }
...
contract CryptoCan is Ownable {
  ...
  function donateEth() payable public requireFundraisingModeOn {
    ...
    uint256 amountOfHearts = msg.value.div(supportedTokens[address(0)]);
    owner().transfer(msg.value);
    uint256 lowestId = heartToken.bulkMint(msg.sender, amountOfHearts);
    _bumpRaised(address(0), msg.value, lowestId);
  }
  ...
  function donateToken(address _token, uint256 _amount) public requireFundraisingModeOn {
    ...
    uint256 amountOfHearts = _amount.div(supportedTokens[_token]);
    uint256 lowestId = heartToken.bulkMint(msg.sender, amountOfHearts);
    _bumpRaised(_token, _amount, lowestId);
  }
  ...
  function _bumpRaised(address _token, uint256 _amount, uint256 lowestId) internal {
    totalRaised[_token] = totalRaised[_token].add(_amount);
    emit Donation(_token, _amount, block.timestamp, lowestId);
  }
