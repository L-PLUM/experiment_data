...
contract VNETTokenPreSale is Ownable {
    ...
    uint256 public ethPriceAutit; // with 18 decimals
    uint256 public vnetSold; // with 18 decimals
    uint256 public vnetSupply = 20 * (10 ** 8) * (10 ** 6); // 20 billion supply
    uint256 public vnetPriceStart = 0.0013 * (10 ** 18); // 0.0013 USD
    uint256 public vnetPriceTarget = 0.0026 * (10 ** 18); // 0.0026 USD
    ...
    event EthPrice(uint256 _ethPriceAutit);


    /**
     * @dev Constructor
     */
    constructor(ERC20Basic _vnetToken, address _wallet) public {
    ...
    function calcRatioNext() private {
        ratioNext = ethPriceAutit.mul(1000000).div(vnetPriceStart.add(vnetPriceTarget.sub(vnetPriceStart).mul(vnetSold.div(vnetSupply))));
    }
}
