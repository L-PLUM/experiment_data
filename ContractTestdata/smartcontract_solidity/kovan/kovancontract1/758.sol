...
contract FutureGame is Owner {
    ...
    uint public investInterval = 10 minutes;  //invest time
    ...
    uint8 public gameType = 1; // 1: once a day 2: Continuous fainting
    // need set Owner
    ...
    constructor(uint256 _timestamp, uint8 _gameType) public {
        name = "test";
        gameType = _gameType;
    ...
    function updateTimer() public onlyWhiteList {
        status = true;
        if (gameType == 1) {
            lockTime[round] = lockTime[round - 1].add(86400);
        } else {
    ...
    function updateIncome(uint _unitIncome, bool _direction) internal {
        ...
        if (_unitIncome > 0) {
            if (_direction) {
                for (i = 0; i < longInvestor.length; i++) {
                    hold = longInvestor[i];
                ...
                for (i = 0; i < shortInvestor.length; i++) {
                    hold = shortInvestor[i];
                    lastIncome[hold] = 0;
    ...
    function setStandardPrice(uint _standardPrice) public onlyWhiteList {
        require(now >= lockTime[round]);
        standardPrice[round] = _standardPrice;
        status = false;
        emit StandardPrice(_standardPrice, now);
    }
    
    function setSettlementPrice(uint _settlementPrice) public onlyWhiteList {
        require(now >= settlementTime[round]);
    ...
    function setInvestInterval(uint _investInterval) public onlyWhiteList {
        investInterval = _investInterval;
    }
    
    function setLockInterval(uint _lockInterval) public onlyWhiteList {
        lockInterval = _lockInterval;
    }
    
    function setGameType(uint8 _gameType) public onlyWhiteList {
        gameType = _gameType;
    }
