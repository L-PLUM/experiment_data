...
// File: contracts/clearing/Permissions/RootPlatformAdministratorRole.sol
...
// File: contracts/clearing/Permissions/AssetTokenAdministratorRole.sol
...
// File: contracts/clearing/Permissions/At2CsConnectorRole.sol
...
// File: contracts/clearing/0_library/DSMathL.sol
...
// File: contracts/clearing/Permissions/YourOwnable.sol
...
// File: contracts/clearing/FeeTable/StandardFeeTable.sol
...
// File: contracts/clearing/FeeTable/FeeTable.sol
...
// File: contracts/clearing/Permissions/WhitelistControlRole.sol
...
// File: contracts/clearing/interface/IWhitelistAutoExtendExpirationExecutor.sol
...
// File: contracts/clearing/interface/IWhitelistAutoExtendExpirationCallback.sol
...
// File: contracts/clearing/Whitelist/Whitelist.sol
...
// File: contracts/clearing/interface/IExchangeRateOracle.sol
...
// File: contracts/clearing/interfaces/IBasicAssetToken.sol
...
// File: contracts/clearing/Permissions/StorageAdministratorRole.sol
...
// File: contracts/clearing/Storage/storagetypes/UintStorage.sol
...
// File: contracts/clearing/Storage/storagetypes/AddrStorage.sol
...
// File: contracts/clearing/Storage/storagetypes/Addr2UintStorage.sol
...
// File: contracts/clearing/Storage/storagetypes/Addr2AddrStorage.sol
...
// File: contracts/clearing/Storage/storagetypes/Addr2BoolStorage.sol
...
// File: contracts/clearing/Storage/storagetypes/BytesStorage.sol
...
// File: contracts/clearing/Storage/storagetypes/Addr2AddrArrStorage.sol
...
// File: contracts/clearing/Storage/storagetypes/StorageHolder.sol
...
// File: contracts/clearing/Storage/AT2CSStorage.sol
...
///////////////////
    ...
    constructor(address clearingStorage) public {
        storageHolder = StorageHolder(clearingStorage);
    }
...
// File: contracts/clearing/0_library/ClearingL.sol
...
///////////////////
    ...
    /// @notice Pay fee on clearing call from Crowdsale.
    ...
    /// @notice Pay fee on clearing call from AssetToken.
...
// File: contracts/clearing/0_library/LibraryHolder.sol

/** @title LibraryHolder holds libraries used in inheritance bellow. */
contract LibraryHolder {
    using ClearingL for ClearingL.Data;

///////////////////
// Variables
///////////////////

    ClearingL.Data internal clearingData;
}

// File: contracts/clearing/1_permissions/PermissionHolder.sol
...
// File: contracts/clearing/2_provider/MainInfoProvider.sol
...
///////////////////
    ...
    function setCrwdTokenAddress(address _crwdToken) public onlyRootPlatformAdministrator {
        clearingData.setCrwdTokenAddress(_crwdToken);
    }

    /// @notice Set exchange rate oracle address.
    /// @param _oracleAddress the address of the ExchangeRateOracle.
    function setOracle(address _oracleAddress) public onlyRootPlatformAdministrator {
        clearingData.setExchangeRateOracle(_oracleAddress);
    }
    ...
    function getFeeTableAddressForPlatform(address _platform) public view returns (address) {
        return clearingData.getFeeTableAddressForPlatform(_platform);
    }   
    ...
    function setAssetTokenRate(address _assetToken, uint256 _rate) public onlyRootPlatformAdministrator {
        clearingData.setAssetTokenRate(_assetToken, _rate);
    }

    /// @notice Set root platform address. Root platform address can receive fees (independent of which Crowdsale/AssetToken).
    /// @param _rootPlatformWallet wallet of root platform.
    function setRootPlatform(address _rootPlatformWallet) public onlyRootPlatformAdministrator {
        clearingData.setRootPlatform(_rootPlatformWallet);
    }
    ...
    function getRootPlatform() public view returns (address) {
        return clearingData.rootPlatformAddress;
    }
    ...
    function setTrustedPlatform(address _platformWallet, bool _trusted) public onlyRootPlatformAdministrator {
        clearingData.setTrustedPlatform(_platformWallet, _trusted);
    }
    ...
    function isTrustedPlatform(address _platformWallet) public view returns (bool) {
        return clearingData.trustedPlatform[_platformWallet];
    }
    ...
    function getPlatformOfCrowdsale(address _crowdsale) public view returns (address) {
        return clearingData.crowdsaleToPlatform[_crowdsale];
    }
    ...
    function setWhitelistContract(address _whitelistAddress) public onlyRootPlatformAdministrator {
        clearingData.whitelist = _whitelistAddress;
    ...
    function getStorageAddress() public view returns (address) {
        return clearingData.getStorageAddress();
    }

    /// @notice Block new connections between AssetToken and Crowdsale (e.g. on upgrade)
    /// @param _isBlockNewActive true if no new AssetTokens or Crowdsales can be added to clearing.
    function setBlockNewState(bool _isBlockNewActive) public onlyRootPlatformAdministrator {
        clearingData.blockNew = _isBlockNewActive;
    }

    /// @notice Gets state of block new.
    /// @return true if no new AssetTokens or Crowdsales can be added to clearing.
    function getBlockNewState() public view returns (bool) {
        return clearingData.blockNew;
    }
}

// File: contracts/clearing/3_manage/ManageAssetToken.sol
...
///////////////////
    ...
    {
        clearingData.addTrustedAssetToken(_ownerWallet, _assetToken, _rate);
    }

    /// @notice Checks if a AssetToken is trusted.
    /// @param _assetToken the address of the AssetToken.
    function checkTrustedAssetToken(address _assetToken) public view returns (bool) {
        return clearingData.checkTrustedAssetToken(_assetToken);
    }
    ...
    function changeActiveTrustedAssetToken(address _assetToken, bool _active) public onlyRootPlatformAdministrator returns (bool) {
        return clearingData.changeActiveTrustedAssetToken(_assetToken, _active);
    }
    ...
    function getFeeKnowingAssetToken(
        ...
        public view returns (uint256)
    {
        return clearingData.getFeeKnowingAssetToken(_assetToken, _from, _tokenAmount, _feeName);
    }
    ...
    function convertEthToTokenAmount(address _crowdsale, uint256 _amountInWei) public view returns (uint256 _tokens) {
        (uint256 tokens, ) = clearingData.convertEthToEurApplyRateGetTokenAmountFromCrowdsale(_crowdsale, _amountInWei);
        return tokens;
    }
}

// File: contracts/clearing/3_manage/ManageFee.sol
...
///////////////////
    ...
    function payFeeKnowingAssetToken(address _assetToken, address _from, uint256 _amount, bytes32 _feeName) internal {
        clearingData.payFeeKnowingAssetToken(_assetToken, _from, _amount, _feeName);
    }
}

// File: contracts/clearing/3_manage/ManageCrowdsale.sol
...
///////////////////
    ...
    function assignCrowdsale(address _assetToken, address _crowdsale, address _platformWallet) 
        ...
        onlyAt2CsConnector 
    {
        clearingData.assignCrowdsale(_assetToken, _crowdsale, _platformWallet);
    }

    /// @notice Checks if a crowdsale is trusted.
    /// @param _crowdsale the address of the crowdsale.
    function checkTrustedCrowdsale(address _crowdsale) public view returns (bool) {
        return clearingData.checkTrustedCrowdsale(_crowdsale);
    }
    ...
    function getFeeKnowingCrowdsale(
        ...
        public view returns (uint256) 
    {
        return clearingData.getFeeKnowingCrowdsale(_crowdsale, _feeTableAddr, _amountInTokensOrEth, _amountRequiresConversion, _feeName);
    }
}

// File: contracts/clearing/3_manage/ManagePlatform.sol
...
///////////////////
    ...
    function checkTrustedPlatform(address _platformWallet) public view returns (bool) {
        return clearingData.checkTrustedPlatform(_platformWallet);
    }
...
// File: contracts/clearing/3_manage/ManageWhitelist.sol
...
///////////////////
    ...
    /// @param _wallet the wallet to check.
    function checkWhitelistedWallet(address _wallet) public returns (bool) {
        clearingData.checkWhitelistedWallet(_wallet);
    }
    ...
    /// @return true if whitelisted.
    function isWhitelistedWallet(address _wallet) public view returns (bool) {
        clearingData.isWhitelistedWallet(_wallet);
    }
}

// File: contracts/clearing/3_manage/ManagerHolder.sol
...
// File: contracts/clearing/interface/ICRWDClearing.sol
...
// File: contracts/clearing/CRWDClearing.sol

/** @title CRWDClearing main contract and n-th child of multi-level inheritance. */
contract CRWDClearing is ManagerHolder, ICRWDClearing {
...
///////////////////
    ...
    constructor(bool _feesEnabled, bool _whitelistEnabled, address _rootPlatformAddress, address _storage) public {
        clearingData.assignStore(_storage);
...
///////////////////
    ...
    function configure(bool _feesEnabled, bool _whitelistEnabled) public onlyRootPlatformAdministrator {
        clearingData.feesEnabled = _feesEnabled;
        clearingData.whitelistEnabled = _whitelistEnabled;
    ...
    function transferParticipantsVerification(address /*_underlyingCurrency*/, address _from, address _to, uint256 _tokenAmount) public returns (bool) {
        ...
        if (clearingData.whitelistEnabled) {
    ...
    function buyFromCrowdsale(address _to, uint256 _amountInWei) public returns (uint256 _tokensCreated, uint256 _overpaidRefund) {
        return clearingData.buyFromCrowdsale(_to, _amountInWei);
    }
    ...
    function assignFromCrowdsale(address _to, uint256 _tokenAmount, bytes8 _tag) external returns (uint256 _tokensCreated) {
        _tokensCreated = clearingData.assignFromCrowdsale(_to, _tokenAmount);
...
////////////////
    ...
    {
        clearingData.rescueToken(_foreignTokenAddress, _to);
    }
}
