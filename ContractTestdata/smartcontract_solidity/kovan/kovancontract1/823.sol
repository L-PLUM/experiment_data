...
contract Carbon {
    uint private cloth_number=0;
    mapping (uint => uint) carbon_foot_list;  //號碼對碳足跡
    mapping (string => uint) total_carbon;      //帳號對總碳排放量
    function record_foot_carbon(uint carbon_foot_num,string account) public {
      //紀錄碳足跡 
      carbon_foot_list[cloth_number]=carbon_foot_num;
    ...
    function read_cloth_number() view public returns(uint){
        return cloth_number;
    }
}
