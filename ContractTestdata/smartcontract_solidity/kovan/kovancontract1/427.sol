...
contract FIH is RevealPrivilege {
    ...
    mapping(uint16 => mapping(uint256 => BonusCode)) revealBonusCodes; // period => code => BonusCode
    mapping(uint16 => uint256[]) bcodes; // period => code

    event Bet(uint16 _currentPeriod, uint256 _orderId, uint256 _code);
    ...
    function reveal(string memory _seed) public onlyAdmins {
        ...
        emit Reveal(currentPeriod, winnerBcode.orderId, winnerBcode.prefix, winnerBcode.code, winnerBcode.addr, halfBonusPool);
    }
    ...
    function getRevealResultPerPeriod(uint16 _period) public view returns(uint8 _prefix, uint256 _orderId, uint256 _code, uint256 _nums, address _addr) {
