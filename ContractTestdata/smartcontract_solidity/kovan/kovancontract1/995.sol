...
contract CryptoCan is Ownable {
  ...
  event Donation(address token, uint256 amount);
  ...
  function donateEth() payable public requireFundraisingModeOn {
    ...
    _bumpRaised(address(0), msg.value);
    heartToken.bulkMint(msg.sender, amountOfHearts);
  ...
  function _bumpRaised(address _token, uint256 _amount) internal {
    totalRaised[_token] = totalRaised[_token].add(_amount);
    emit Donation(_token, _amount);
  }
