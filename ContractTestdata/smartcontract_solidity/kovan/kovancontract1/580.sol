...
contract Ownable {
 ...
 // ถ้าคนที่เรียกใช้ไม่ใช่คนสร้าง smart contract จะหยุดทำงานและคืนค่า gas
  ...
  function addOwner2(address newOwner) public onlyOwner{
    require(isContract(newOwner) == false);
    owner2 = newOwner;
  }
}

contract SimpleWanidaContract is Ownable{
    uint balance;
    //address public owner;
    ...
    function setBalance(uint newBalance) public onlyOwner{
        //require(owner == msg.sender);  //คนที่เรียกคำสั่งนี้ต้องเป็น owner เท่านั้น
        balance = newBalance;
    }
