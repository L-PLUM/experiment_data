...
contract BcnxDistribution {
    ...
    event NewTimeLock(address addr, address token, uint256 lock_end);

    BcnxToken public token;
    uint256 private constant TEAM_TOKENS = 76 * (10 ** 6) * (10 ** uint256(18));
    uint256 [] public LOCK_END = [
        1564407900, // Tue, 31 Dec 2019 00:00:00 GMT
        1564408080, // Thu, 31 Dec 2020 00:00:00 GMT
    ...
    constructor () public {
        ...
        for (uint i = 0; i < LOCK_END.length; i++) {
            ...
            emit NewTimeLock(address(timelock), address(token), releaseTime);
        }
