...
/**
 * @dev Allocation for reserve VOKENs.
 */
library Allocations {
    ...
    function available(Allocation storage self) internal view returns (uint256) {
        if (now > self.timestamp) {
            ...
            return self.allocation.mul(__passed).div(self.steps);
        }
    ...
    function reserved(Allocation storage self) internal view returns (uint256) {
        return self.allocation.sub(available(self));
    }
}
...
contract NewVoken is Ownable, Pausable, IERC20 {
    ...
    mapping (address => mapping (address => bool)) private _mappingAddressAllocation;
    ...
    function _getAvailableAmount(address account, uint256 amount) internal view returns (uint256) {
        uint256 __available = availableOf(account);
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if (_rejectNonWhitelistTransaction && sender != address(_whitelist) && recipient != address(_whitelist)) {
    ...
    function _mintWithAllocation(address account, uint256 amount, IAllocation allocationContract) internal {
        ...
        if (!_mappingAddressAllocation[account][address(allocationContract)]) {
            _allocations[account].push(allocationContract);
            _mappingAddressAllocation[account][address(allocationContract)] = true;
        }
