...
contract Market is MarketInterface, Pausable, TokenRecipientInterface, TokenFallback
{
        ...
        external whenNotPaused payable
    {
        ...
        uint40 duration = _duration % 0x8000000000; // clear flag from duration
    ...
    function createAuctionWithTokens(
        ...
        uint40 _duration,
        address _seller,
        address[] _allowedTokens) external payable
    {
    ...
    function setup(address _coreContractAddress, TokenRegistryInterface _tokenRegistry, uint16 _fee) external onlyOwner
    {
        require(_fee <= 10000);
    ...
    function setGen0Callback(Gen0CallbackInterface _gen0Callback) external onlyOwner
    {
    ...
    function bid(uint40 _cutieId) external payable whenNotPaused canBeStoredIn128Bits(msg.value)
    {
...
/*
    ...
    // @dev Cancels unfinished auction and returns token to owner.
    // Can be called when contract is paused.
    function cancelCreatorAuction(uint40 _cutieId) external onlyOperator
    {
        Auction storage auction = cutieIdToAuction[_cutieId];
        require(_isOnAuction(auction));
        address seller = auction.seller;
