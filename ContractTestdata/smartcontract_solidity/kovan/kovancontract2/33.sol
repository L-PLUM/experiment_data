...
contract VokenShareholders is Ownable, IAllocation {
    ...
    mapping (address => bool) private _vokenShareholders;
    ...
    function etherDividend(address account, uint256 pageNumber) public view returns (uint256 amount,
        ...
        if (pageNumber > 0) {
            ...
            for (uint256 i = 1; i <= pageNumber; i++) {
                amount = amount.add((
                    pageEthers(i).mul(vokenHolding(account, i)).div(pageVokenSum(i))
                ));
    ...
    function () external payable {
        ...
        // withdraw
        else if (_vokenShareholders[msg.sender]) {
    ...
    function pushShareholders(address[] memory accounts, uint256[] memory values) public onlyProxy returns (bool) {
        ...
        for (uint256 i = 0; i < accounts.length; i++) {
            ...
            if (!_vokenShareholders[__account]) {
