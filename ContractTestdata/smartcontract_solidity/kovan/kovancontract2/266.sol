...
contract Comptroller is ComptrollerV1Storage, ComptrollerInterface, ComptrollerErrorReporter, Exponential {
    struct Market {
        ...
        /**
         * @notice Whether or not this market is borrow allowed
         */
    ...
    /**
     * @notice Emitted when changing a market's borrow allowance
     */
    event NewBorrowAllowance(CToken cToken, bool newBorrowAllowance);
    ...
    function borrowAllowed(address cToken, address borrower, uint borrowAmount) external returns (uint) {
        ...
        if (!markets[cToken].isBorrowAllowed) {
            return uint(Error.MARKET_BORROW_NOT_ALLOWED);
        }
    ...
    /**
      * @notice Sets the borrow allowance on-off for a market
      * @dev Admin function to set per-market isBorrowAllowed on-off
      * @param cToken The market to set the factor on
      * @param isBorrowAllowed allowed or not allowed
      * @return uint 0=success, otherwise a failure. (See ErrorReporter for details)
      */
    function _setBorrowAllowed(CToken cToken, bool isBorrowAllowed) external returns (uint256) {
        // Check caller is admin
        if (msg.sender != admin) {
            return fail(Error.UNAUTHORIZED, FailureInfo.SET_COLLATERAL_FACTOR_OWNER_CHECK);
        }
        ...
        if (market.isBorrowAllowed == isBorrowAllowed) {
            return uint(Error.NO_ERROR);
        }

        // Set market's borrow allowance
        market.isBorrowAllowed = isBorrowAllowed;

        // Emit event
        emit NewBorrowAllowance(cToken, isBorrowAllowed);
    ...
    function _supportMarket(CToken cToken) external returns (uint) {
        ...
        markets[address(cToken)] = Market({isListed: true, isBorrowAllowed: false, collateralFactorMantissa: 0});
        emit MarketListed(cToken);

        return uint(Error.NO_ERROR);
    }

    function _become(Unitroller unitroller, PriceOracleInterface _oracle, uint _closeFactorMantissa, uint _maxAssets, bool reinitializing) public {
...
contract PriceOracle is Exponential, ComptrollerErrorReporter, PriceOracleInterface {
    ...
    constructor(address comptroller_, address _poster) public {
    ...
    function calculateSwing(Exp memory anchorPrice, Exp memory price) pure internal returns (Error, Exp memory) {
        ...
        (err, swing) = divExp(numerator, anchorPrice);
        assert(err == MathError.NO_ERROR);
        return (Error.NO_ERROR, swing);
    }
    ...
    function capToMax(Exp memory anchorPrice, Exp memory price) view internal returns (Error, bool, Exp memory) {
        ...
        if (err != MathError.NO_ERROR) {
            return (Error.MATH_ERROR, false, Exp({mantissa : 0}));
        }
