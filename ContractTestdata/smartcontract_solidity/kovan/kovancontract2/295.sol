...
contract BlockchainCutiesERC1155 is ERC1155MixedFungible, Operators, ERC1155Metadata_URI, IERC1155Mintable {
    ...
    function mintNonFungibleSingleShortCount(uint128 _type, address _to, uint _count) external onlyOperator {
        uint tokenType = (uint256(_type) << 128) | (1 << 255);
    ...
    function mintNonFungibleShort(uint128 _type, address[] _to) external onlyOperator {
        uint tokenType = (uint256(_type) << 128) | (1 << 255);
        _mintNonFungible(tokenType, _to);
    }

    function mintNonFungible(uint256 _type, address[] _to) external onlyOperator {
    ...
    function mintFungible(uint256 _id, address[] _to, uint256[] _quantities) external onlyOperator {
    ...
    function setURI(string _uri, uint256 _id) external onlyOperator {
        emit URI(_uri, _id);
    }
    ...
    function ownerOfNftShort(uint128 _type, uint128 _index) external view returns (address) {
        uint tokenType = (uint256(_type) << 128) | (1 << 255) | _index;
        return _ownerOf(tokenType);
    }
}
