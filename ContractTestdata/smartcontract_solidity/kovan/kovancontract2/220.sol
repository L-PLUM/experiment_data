...
contract Att is ERC20, Pausable {
...
contract AttLock {
    using SafeMath for uint256;
    Att internal AttToken;
    
   uint256 internal genesisTime= 1564076392;//固定时间  秒 2018-12-27 09:00:00;  //开始时间设为固定值
    

    uint256 internal ONE_MONTHS = 300;  //1个月的秒
        ...
        for(uint256 i = 0; i < beneficiaryClaims.length; i++){
            ...
            if(cur_claim.claimed == false){
                if(cur_claim.delay.add(genesisTime) < block.timestamp){
                    ...
                    require(AttToken.transfer(msg.sender, amount));
...
contract attFound is AttLock {
    using SafeMath for uint256;
    
    constructor(Att _AttToken) public {
        AttToken = _AttToken;
        
        
        
        beneficiaryAddress = 0x2aD8Aa4764700a0D17C3216DA1bd7B14eEF6d78A ;
