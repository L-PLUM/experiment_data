...
contract DerivativesEntry is Derivatives
{
    ...
    function test_delegate(uint256 value) external
    {
        (bool success,) = service.delegatecall(abi.encodeWithSignature("delegate_function(uint256)", value));
        require(success);
    }
    
    function test_delegate2(uint256 value) external
    {
        (bool success,) = service.delegatecall(abi.encode("delegate_function(uint256)", value));
        require(success);
    }
