...
/**
  * @title The SEAL-DEFI InterestRateModel Interface
  * @author SEAL
...
/**
 * @title SEAL-DEFI's SToken Contract
 * @notice Abstract base for STokens
 * @author SEAL
 */
contract SToken is EIP20Interface, Exponential, TokenErrorReporter, ReentrancyGuard {
    /**
     ...
     */
    event AccrueInterest(uint interestAccumulated, uint borrowIndex, uint totalBorrows, uint totalReserves);

    /**
     * @notice Event emitted when tokens are minted
     */
    event Mint(address minter, uint mintAmount, uint mintTokens, uint accountTokens, uint totalSupply);

    /**
     * @notice Event emitted when tokens are redeemed
     */
    event Redeem(address redeemer, uint redeemAmount, uint redeemTokens, uint accountTokens, uint totalSupply);

    /**
     * @notice Event emitted when underlying is borrowed
     */
    event Borrow(address borrower, uint borrowAmount, uint borrowIndex, uint accountBorrows, uint totalBorrows);

    /**
     * @notice Event emitted when a borrow is repaid
     */
    event RepayBorrow(address payer, address borrower, uint repayAmount, uint borrowIndex, uint accountBorrows, uint totalBorrows);
    ...
    function accrueInterest() public returns (uint) {
        ...
        emit AccrueInterest(vars.interestAccumulated, vars.borrowIndexNew, totalBorrows, totalReserves);
    ...
    function mintFresh(address minter, uint mintAmount) internal returns (uint) {
         ...
         *  On success, the sToken holds an additional mintAmount of cash.
        ...
        emit Mint(minter, mintAmount, vars.mintTokens, vars.accountTokensNew, vars.totalSupplyNew);
     ...
     * @param redeemTokensIn The number of sTokens to redeem into underlying (only one of redeemTokensIn or redeemAmountIn may be zero)
     * @param redeemAmountIn The number of sTokens to redeem into underlying (only one of redeemTokensIn or redeemAmountIn may be zero)
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function redeemFresh(address payable redeemer, uint redeemTokensIn, uint redeemAmountIn) internal returns (uint) {
        ...
        emit Redeem(redeemer, vars.redeemAmount, vars.redeemTokens, vars.accountTokensNew, vars.totalSupplyNew);
    ...
    function borrowFresh(address payable borrower, uint borrowAmount) internal returns (uint) {
        ...
        emit Borrow(borrower, borrowAmount, borrowIndex, vars.accountBorrowsNew, vars.totalBorrowsNew);
    ...
    function repayBorrowFresh(address payer, address borrower, uint repayAmount) internal returns (uint) {
        ...
        emit RepayBorrow(payer, borrower, vars.repayAmount, borrowIndex, vars.accountBorrowsNew, vars.totalBorrowsNew);
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function liquidateBorrowInternal(address borrower, uint repayAmount, SToken sTokenCollateral) internal nonReentrant returns (uint) {
        uint error = accrueInterest();
        if (error != uint(Error.NO_ERROR)) {
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function liquidateBorrowFresh(address liquidator, address borrower, uint repayAmount, SToken sTokenCollateral) internal returns (uint) {
        ...
        (uint amountSeizeError, uint seizeTokens) = comptroller.liquidateCalculateSeizeTokens(address(this), address(sTokenCollateral), repayAmount);
        ...
        if (seizeTokens > sTokenCollateral.balanceOf(borrower)) {
        ...
        uint seizeError = sTokenCollateral.seize(liquidator, borrower, seizeTokens);
        require(seizeError == uint(Error.NO_ERROR), "token seizure failed");

        /* We emit a LiquidateBorrow event */
        emit LiquidateBorrow(liquidator, borrower, repayAmount, address(sTokenCollateral), seizeTokens);

        /* We call the defense hook */
        comptroller.liquidateBorrowVerify(address(this), address(sTokenCollateral), liquidator, borrower, repayAmount, seizeTokens);
...
/**
 * @title SEAL-DEFI's SErc20 Contract
 * @notice STokens which wrap an EIP-20 underlying
 * @author SEAL
 */
contract SErc20 is SToken {
    ...
    SToken(comptroller_, interestRateModel_, initialExchangeRateMantissa_, name_, symbol_, decimals_) {
        // Set underlying
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function liquidateBorrow(address borrower, uint repayAmount, SToken sTokenCollateral) external returns (uint) {
        return liquidateBorrowInternal(borrower, repayAmount, sTokenCollateral);
    }
