...
{
    event on_deposit(address target, uint256 value);
    event on_withdrawal(address target, uint256 value);
    event on_create_order(address target, uint256 quantity_usd, uint256 position_type);
    event on_liquidate(address target);
	...
	function liquidate(address target) internal
	{
		...
		if(clients[target].position_type == 0)
		{
			...
			if(price_in_usd_cents < liquidation_price)
		    {
		        reduce_position(target, clients[target].quantity_usd, 1, 1);
		        emit on_liquidate(target);
		    }
		}else{
			uint256 base_liquidation_price = clients[target].price_in_usd_cents*quantity_unit_cents/(quantity_unit_cents - available_unit_cents);
			uint256 liquidation_price = base_liquidation_price - calculate_margin(clients[target].price_in_usd_cents);
			...
			if(price_in_usd_cents > liquidation_price)
		    {
		        reduce_position(target, clients[target].quantity_usd, 0, 1);
                emit on_liquidate(target);
		    }
		}
	}
	...
	function extend_position(address target, uint256 quantity_usd, uint256 position_type) internal
	{
		...
		require(initial_margin_unit_cents <= balance_unit_cents, "insufficient funds");
		
		uint256 fee_unit_cents = calculate_fee_for_usd(quantity_usd, price_in_usd_cents);
		
		require(fee_unit_cents <= balance_unit_cents, "insufficient funds");
	...
	function reduce_position(address target, uint256 quantity_usd, uint256 position_type, uint256 force) internal
	{
		...
		uint256 lose_unit_cents = convert_to_unit_cents_by_price(delta_usd_cents, mem.price_in_usd_cents);
		...
		if(profit_unit_cents >= lose_unit_cents)
		{
		    ...
		    if(hard_lose > mem.hard_balance_unit_cents) hard_lose = mem.hard_balance_unit_cents;
		...
		require(force == 1 || fee_unit_cents <= balance_unit_cents, "insufficient funds");
    ...
    function deposit() external payable
    {
	    ...
	    if(msg.sender != master)
	    {
	        ...
	        emit on_deposit(msg.sender, msg.value);
	    }
    }
    ...
    function withdrawal(uint256 value) external
    {
		...
		uint256 balance_unit_cents = client.hard_balance_unit_cents + client.soft_balance_unit_cents;
		
		uint256 quantity_usd_cents = client.quantity_usd*100;
		if(quantity_usd_cents > 0)
		{
            uint256 lose_unit_cents = convert_to_unit_cents_by_price(quantity_usd_cents, client.price_in_usd_cents);
		    uint256 margin_unit_cents = calculate_margin(lose_unit_cents);
		    
		    balance_unit_cents = reduce(balance_unit_cents, margin_unit_cents);
		    
		    uint256 profit_unit_cents = convert_to_unit_cents_by_price(quantity_usd_cents, price_in_usd_cents);
		    if(lose_unit_cents > profit_unit_cents) balance_unit_cents = reduce(balance_unit_cents, lose_unit_cents-profit_unit_cents);
		}
		
        require(value <= balance_unit_cents, "insufficient funds for withdrawal");
		
		uint256 hard_value = min(client.hard_balance_unit_cents, value);
		uint256 soft_value = value - hard_value;
		
		uint256 contract_soft_available = address(this).balance - hard_reserved_unit_cents;
		
		require(soft_value <= contract_soft_available, "insufficient funds");
		
		hard_reserved_unit_cents -= hard_value;
		client.hard_balance_unit_cents -= hard_value;
		client.soft_balance_unit_cents -= soft_value;
		
		msg.sender.transfer(value);
		
	    emit on_withdrawal(msg.sender, value);
    }
    ...
    {
        require(service != address(0), "service not initialized");
        require(quantity_usd > 0, "quantity must be positive");
    	...
    	emit on_create_order(msg.sender, quantity_usd, 0);
    }
    ...
    function create_order_short(uint256 quantity_usd) external
    {
		...
		if(clients[msg.sender].position_type == 1 || clients[msg.sender].quantity_usd == 0) extend_position(msg.sender, quantity_usd, 1);
     	...
     	emit on_create_order(msg.sender, quantity_usd, 1);
   }
}
