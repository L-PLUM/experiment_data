...
**** M""MMM""MMM""M                   dP                      dP ****
**** M  MMM  MMM  M                   88                      88 ****
**** M  MMP  MMP  M .d8888b. .d8888b. 88  .dP  .d8888b. .d888b88 ****
**** M  MM'  MM' .M 88'  `88 88'  `"" 88888"   88ooood8 88'  `88 ****
**** M  `' . '' .MM 88.  .88 88.  ... 88  `8b. 88.  ... 88.  .88 ****
**** M    .d  .dMMM `88888P8 `88888P' dP   `YP `88888P' `88888P8 ****
**** MMMMMMMMMMMMMM                                              ****
***********************************************************TOKEN*****
*********************************************************************
***     __                        ___                             ***
***    / /_  __  ___________     ( _ )      ___  ____ __________  ***
***   / __ \/ / / / ___/ __ \   / __ \/|   / _ \/ __ `/ ___/ __ \ ***
***  / /_/ / /_/ / /  / / / /  / /_/  <   /  __/ /_/ / /  / / / / ***
*** /_.___/\__,_/_/  /_/ /_/   \____/\/   \___/\__,_/_/  /_/ /_/  ***
*********************************************************************
*********************************************************************
*/
...
contract WACKv6 is ERC20Detailed 
{
    ...
    string constant tokenName = "WUUG HODL Earning";//"WACK Burn & Earn";
    string constant tokenSymbol = "WUUG";//"WACK"; 
    ...
    mapping (address => bool) public excludedFromEarning; //exchanges/other contracts will be excluded from staking
    ...
    constructor() public ERC20Detailed(tokenName, tokenSymbol, tokenDecimals) 
    {
        ...
        excludedFromEarning[msg.sender] = true;
        excludedFromEarning[address(this)] = true;
    ...
    function _mint(address account, uint256 value) internal 
    {
        ...
        if(!excludedFromEarning[account])
        {
    ...
    *   the receiver gets 90% of the sent value
    *   10% are split to be burnt and distributed to holders
    */
    function _executeTransfer(address from, address to, uint256 value) private
    {
        ...
        uint256 fourPercent = value.mul(4).div(100);
    ...
    {
        _savedRewards[staker] = viewUnpaidEarnings(staker);
        _totalRewardsPerUnit_positions[staker] = _totalRewardsPerUnit;
    }
    
    //get all rewards that have not been claimed yet
    function viewUnpaidEarnings(address staker) public view returns (uint256)
    {
        if(excludedFromEarning[staker])
    ...
    function payoutWACKEarnings() public
    {
        ...
        //update full units staked
        if(!excludedFromEarning[msg.sender])
        {
            uint256 fus_total = fullUnitsStaked_total;
    ...
    //exchanges or other contracts can be excluded from receiving stake rewards ("true" or "false" input)
    function excludeAddressFromStaking(address excludeAddress, bool exclude) public
    {
        ...
        require(exclude != excludedFromEarning[excludeAddress]);
        updateRewardsFor(excludeAddress);
        excludedFromEarning[excludeAddress] = exclude;
