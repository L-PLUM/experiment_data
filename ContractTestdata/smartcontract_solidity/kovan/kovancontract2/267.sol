...
contract MicroShopCandyBox is Owned{
	...
	// used by receiveApproval
	function _pledge(address customer, uint value) public {
	...
	function pledge(uint value) public {
		require(maxPledges >= totalPledges.add(value));
		
		claimCandies();
		microToken.transferFrom(msg.sender, address(this), value);
		totalPledges = totalPledges.add(value);
		pledgeDate[msg.sender] = now;
		pledges[msg.sender] = pledges[msg.sender].add(value);
		emit PledgeLogs(msg.sender, value);
	}
    ...
    // ------------------------------------------------------------------------
    // receiveApproval, data=0x01 for call _pledge
    // ------------------------------------------------------------------------		
    ...
    // admin functions
    // ------------------------------------------------------------------------	
	function setParam(uint _minimu_days, uint _max_days, uint _base_rate, uint _candy_rate, uint _max_pledges) public onlyOwner{
