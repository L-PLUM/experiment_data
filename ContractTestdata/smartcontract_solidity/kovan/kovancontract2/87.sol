...
contract VokenWhitelist is Ownable {
    ...
    constructor () public {
        ...
        _referee[msg.sender] = address(this);
        _counter = 1;

        emit SignUpEnabled();
        emit SignedUp(msg.sender, address(this));
     ...
     * @dev Batch.
     *
     * Can only be called by the proxy.
     */
    ...
    function _distributeVokens(address account) internal {
        ...
        for(uint i = 0; i < _vokenRewardsArr.length; i++) {
            ...
            if (__cursor != __receiver && _referrals[__receiver].length > i) {
                _voken.transfer(__receiver, _vokenRewardsArr[i]);
        ...
        _voken.transfer(account, _vokenRefund);
    }
