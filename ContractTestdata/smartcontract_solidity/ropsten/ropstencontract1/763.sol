...
//
// * TomoTRC20Dice's smart contract, deployed at 0xac4888d19be9d02a3a0ad31fdcca5fd9a8f880db (Ropsten testnet).
//
...
contract TomoTRC20Dice {
    ...
    uint24[3] BACCARAT_BET_MULTIPLERS = [90, 20, 19]; // [0]Tie: 9x, [1]Player: 2x, [2]Banker: 1.9x
    ...
    function getDiceWinAmount(uint amount, uint modulo, uint rollUnder) private view returns (uint winAmount, uint _jackpotFee, uint _houseEdge, uint betAmount) {
        ...
        uint donationPermille = BankRollerInterface(bankRoller).donationPermille();
        uint donationAmount = (donationPermille > 0) ? amount * donationPermille / 1000 : 0;

        require(_houseEdge + donationAmount + _jackpotFee <= amount, "Bet doesn't even cover houseEdge, donation and jackpotFee.");

        betAmount = amount - _houseEdge - donationAmount - _jackpotFee;
        if (modulo == TOMOLOTTO_4D_MODULO) {
            winAmount = betAmount * LOTTO_PRIZE_4D[4] / 10;
        } else if (modulo == BACCARAT_MODULO) {
            winAmount = betAmount * BACCARAT_BET_MULTIPLERS[rollUnder] / 10;
        } else {
    ...
    // get baccarat win
    function getBaccaratWinAmount(uint modulo, uint rollUnder, uint dice, uint betAmount) private returns (uint winAmount) {
        ...
        if (playerSum == bankerSum) return (rollUnder == 0) ? betAmount * BACCARAT_BET_MULTIPLERS[rollUnder] / 10 : 0;
        else if (playerSum > bankerSum) return (rollUnder == 1) ? betAmount * BACCARAT_BET_MULTIPLERS[rollUnder] / 10 : 0;
        else return (rollUnder == 2) ? betAmount * BACCARAT_BET_MULTIPLERS[rollUnder] / 10 : 0;
    }
