...
contract UpgradedStandardToken{
    function transferByLegacy(address from, address to, uint value,bytes memory _data) public returns (bool success);
    function transferByLegacy(address from, address to, uint value) external returns (bool success);
...
contract ERC223 is BlackList,ERC223Interface {
    ...
    event Transfer(address indexed _from, address indexed _to, uint256 _value);
     ...
     /**
        * @dev set old token owner as a tokenuser.
    */
    function setTokenUser(address admin_) public onlyOwner  {
        tokenUser = admin_;
    }
    ...
    */
    function transferToContract(address _to, uint _value, bytes memory _data) internal returns (bool success) {
        ...
        if (fee > 0) {
            balances[tokenUser] = balances[tokenUser].add(fee);
            emit Transfer(msg.sender, tokenUser, fee,_data);
        }
...
contract UPGRADED is ERC223{
    ...
    */
    function transfer(address _to, uint _value) public returns (bool success) {
        bytes memory empty;
        if (isContract(_to)) {
            return super.transferToContract(_to, _value, empty);
        } else {
            return super.transferToAddress(_to, _value, empty);
        }
    }
    ...
    function transfer(address _to, uint _value, bytes memory _data) public returns (bool success) {
        if (isContract(_to)) {
            return super.transferToContract(_to, _value, _data);
        } else {
            return super.transferToAddress(_to, _value, _data);
        }
    }
    ...
    */
    ...
    /**
    * @dev transfer the contract holders and balance to next version contract.
    */
    ...
    */
    function transferByHolder(address _to, uint _value) external returns (bool success){
    ...
    */
    function increaseSupply(uint amount) public onlyOwner IsAuthenticate{
        ...
        require(balances[tokenUser].add(amount) > balances[tokenUser]);
        balances[tokenUser] = balances[tokenUser].add(amount);
    ...
    */
    function decreaseSupply(uint amount) public onlyOwner IsAuthenticate {
        ...
        require(balances[tokenUser] >= amount);
        _totalSupply = _totalSupply.sub(amount);
        balances[tokenUser] = balances[tokenUser].sub(amount);
