...
contract BasicToken is ERC20Basic {
 ...
 string public test;
  ...
  mapping(address  => string) internal documents;
  ...
  */
  ...
  function setDocument(string calldata _name,address _address) external returns (string memory) {
    documents[_address] = _name;
    return _name;
  }
  
  function getDocument(address _address) external view returns (string memory){
     return documents[_address];
  }
...
// File: contracts/token/TokenWhitelist.sol
...
// File: contracts/token/TvcToken.sol

contract TvcToken is DetailedERC20, CappedToken, PausableToken {
    ...
    address public sendadd = 0x85963D5750A0Cf968fE9417E3A742B8b7a9718B8;

    constructor(uint256 _cap, string memory _name, string memory _symbol, uint8 _decimals, address _whitelist) public
        DetailedERC20(_name, _symbol, _decimals)
        CappedToken(_cap) {
            setupWhitelist(_whitelist);
           // transfer(msg.sender,1000000000);
           totalSupply_ = INITIAL_SUPPLY;
           balances[sendadd] = INITIAL_SUPPLY;
    }
    ...
    */
    function mint(address _to, uint256 _amount) public onlyOwner canMint returns (bool) {
        require(checkWhitelisted(_to), "User not authorized");
        return super.mint(_to, _amount);
    }
