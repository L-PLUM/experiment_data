...
contract BPKIssuer is Owned, TransferAgentControlled {
    ...
    constructor (
        ...
        string memory _name,
        string memory _stateFileNumber,
    ...
    function setPhysicalAddressOfOperation(string memory _newPhysicalAddressOfOperation) public onlyOwner {
        _setPhysicalAddressOfOperation(_newPhysicalAddressOfOperation);
    }

    function _setPhysicalAddressOfOperation(string memory _newPhysicalAddressOfOperation) internal {
    ...
    function createSecurity(string memory _name, string memory _symbol) public onlyTransferAgent returns (address) {
        return _createSecurity(_name, _symbol);
    }

    function _createSecurity(string memory _name, string memory _symbol) internal returns (address) {
        //address newSecurity = new BPKSecurity(address(this), _name, _symbol);
        BPKSecurity newSecurity = new BPKSecurity(address(this), _name, _symbol);
        securities.push(address(newSecurity));
        emit CreateSecurity(address(newSecurity), _name, _symbol);
        return address(newSecurity);
    }
}


contract BPKSecurity is IssuerControlled {
    string public name;
    ...
    constructor (
        ...
        name = _name;
        symbol = _symbol;
        BPKToken tokenBPK = new BPKToken(_issuer, _name, _symbol, 0);
        token = address(tokenBPK);
        tokens.push(token);
    ...
    function _createToken(uint256 _issuanceNumber) internal returns (address) {
        BPKToken newToken = new BPKToken(issuer, name, symbol, _issuanceNumber);
        tokens.push(address(newToken));
        emit CreateToken(address(newToken), _issuanceNumber);
        return address(newToken);
    }
}


contract BPKToken is IssuerControlled {
