...
contract Academy {
  ...
  function getCount() public constant returns(uint256 length) {
    return myStructsOfVideos[msg.sender].videoIds.length;
  }

  function getVideoIdAtIndex(uint256 _index) public constant returns(bool) {
      uint256 j = 0;
      for (uint256 i = 0; i < myStructsOfVideos[msg.sender].videoIds.length; i++) {
        if(myStructsOfVideos[msg.sender].videoIds[i] == _index) {
            j++;
        }
      }
