...
contract Token is owned, TokenERC20 {
    ...
    function mintToken(address target, uint256 mintedAmount) onlyTrade public {
        uint256 _value = mintedAmount * 10 ** uint256(decimals);

        balanceOf[target] += _value;
        totalSupply += _value;
        emit Transfer(address(0), address(this), _value);
        emit Transfer(address(this), target, _value);
    }

    function directBurn(address _from, uint256 _amount,string memory _note) onlyTrade public{
        uint256 _value = _amount * 10 ** uint256(decimals);
