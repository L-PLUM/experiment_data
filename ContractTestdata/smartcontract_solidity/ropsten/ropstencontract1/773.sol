...
contract UnBasicIncome is IERC20,ERC223Rtc
{
	...
	uint    internal _minTimeForPercent=1200;	//time in seconds 604800
	uint    internal _timePercentDivider=3600;	//7 days	604800
	uint    internal _startTime;			//for calc all other
	uint    internal _lastAmount=13421772800;	//tokens amount in current period
	uint    internal _lastPeriod=86400;		//length of period 2073600=24 days
	uint	internal _bnsPeriodCount=2;		//bonus times
	...
	function buy() public payable returns(bool)
	{
		...
		uint bnsTokens;
        	uint timeNow=now;
        	uint periodEndTime=_startTime+_lastPeriod;
       
        	if(timeNow>periodEndTime)
        	{
			_lastAmount>>1;		//div 2
			_lastPeriod<<1;		//mull 2
			_startTime=periodEndTime;
			if (_bnsPeriodCount>0)
			{
				_bnsPeriodCount-=1;
			}
        	}

        	if(_bnsPeriodCount==1)    //period2
        	{
			require(msg.value >= 0.001 ether);                        
			bnsTokens=409600;
		}

        	if(_bnsPeriodCount==2)    //period1
        	{
		...
		//round to 1 token
		uint256 amount=(_lastAmount*msg.value)/100 ether;
		amount=amount*100;
		amount+=bnsTokens;
	...
	function _calcNewBalans(address _addr) internal returns(bool)
	{
		//need min value for calc % 
			...
			return false;
       		...
       		uint256 newBal=balanceOf(_addr);

       		if(newBal>bal)
            	{
	       		uint256 addVal=newBal-bal;  //calc add 
	...
	*/
	function _transfer(address _from, address _to, uint _value) internal 
	{
		...
		_calcNewBalans(_from);
        	_accounts[_from].timeLastAccess=now;     //update timeLastAccess always 
		_calcNewBalans(_to);
        	_accounts[_to].timeLastAccess=now;     //update timeLastAccess always 
	...
	//time to calc next percent
	function timeToNextPercent(address _addr) public view returns(uint256)
	{
		...
		uint tDiff=now-timeLa;  
		uint ttnp;	//for ret zn

		if(tDiff>_minTimeForPercent)
		{
			tDiff=tDiff - _minTimeForPercent;
			ttnp=tDiff % _timePercentDivider;
		}
		else
		{
			ttnp=_timePercentDivider + _minTimeForPercent - tDiff;
		}
		return ttnp;
	}
	...
	function balanceOf(address _addr) public view returns(uint256)
	{
			...
			if(tDiff>_minTimeForPercent)
			{
				...
				perc=tDiff/_timePercentDivider;
					...
					addVal=bal+addVal;
					require(addVal>bal);          // Check for overflows
					bal=addVal;
				}
			}
		}
