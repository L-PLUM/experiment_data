...
contract lendgeralphaToken is ERC20Interface, Owned, SafeMath {
    bytes32 public constant symbol = "LNGR";
    bytes32 public constant name = "lendgeralpha";
    ...
    uint96 public constant hardCap = 1000000000000000000000000000;        // cannon issue more than this many tokens
    uint96 public payoutSize = 100000000000000;     // send ether to owner whenever the contract has this much ether
    uint96 public constant privateTokens = 200000000000000000000000000;  // number of tokens the owner starts with
    ...
    uint96[3] public tierCaps = [200000000010000000000000000, 200000000030000000000000000, 1000000000000000000000000000];
    uint48[3] public tierBonusNumerator = [12, 11, 10];
    uint48[3] public tierBonusDenominator = [10, 10, 10];
    ...
    // ------------------------------------------------------------------------
    function setPayoutSize(uint96 value) public onlyOwner returns (bool) {
        payoutSize = value;
        return true;
    }
    ...
    // ------------------------------------------------------------------------
    function purchaseTokens(uint ethToSpend, uint tier, uint currentSupply /**, uint currentExchangeRate */) private returns (uint ethRemaining, uint tokensBought, bool hitCap) {
        uint numerator = tierBonusNumerator[tier];
        uint denominator = tierBonusDenominator[tier];
        uint cap = tierCaps[tier];
        uint tokens = safeMul(ethToSpend, /**currentE*/exchangeRate);
        tokens = safeMul(tokens, numerator) / denominator;
        uint newSupply = safeAdd(currentSupply, tokens);
        uint refund = 0;
        if (newSupply > cap) {
            tokens = safeSub(cap, currentSupply);
            uint spent = safeMul(tokens, denominator) / numerator / /**currentE*/exchangeRate;
    ...
    function () external payable {
        ...
        while (ethRemaining > 0 && newSupply < hardCap) {
            (ethRemaining, tokensBought, hitCap) = purchaseTokens(ethRemaining, newTier, newSupply); //, currentExchangeRate);
            if (hitCap) {
                ++newTier;
            }
