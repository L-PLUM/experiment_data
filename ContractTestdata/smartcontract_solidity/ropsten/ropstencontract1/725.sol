...
//
// * Mcashdice's Bank Roller smart contract, deployed at 0x370b266ffa7cd9c0e5c2e7d56e6dcffc5a6c8a4d (Ropsten - Ether tesnet).
//
...
contract BankRoller {
    ...
    function setMaxCapIndividualDeposit(address bankRoller, uint maxCap) public onlyOwner {
    ...
    function bankRollerBalance(address bankRoller) public view returns (uint256 _balance) {
        ...
        if (deposit == 0) {
            ...
            require((contractBalance * deposit * 99) / 99 / deposit == contractBalance, "Multiply operation is not safe");
            _balance = contractBalance * deposit * 99 / totalDeposit / 100; // 1% for reserve - cover lockInBet, gas, etc
        }
    }
    ...
    function bankRollerWithdraw(uint8 withdrawPercent) external {
        ...
        uint contractBalance = ERC223Interface(trc20Token).balanceOf(address(this));
        require((contractBalance * currentDeposit * withdrawPercent * 99) / 99 / withdrawPercent == contractBalance * currentDeposit, "Multiply operation is not safe");
        if (withdrawPercent == 100) {
            removeDepositor(msg.sender);
        }
        uint withdrawAmount = contractBalance * currentDeposit * withdrawPercent * 99 / totalDeposit / 100 / 100; // 1% for reserve - cover lockInBet, gas, etc
    ...
    // Owner can remove and refund a bank-rollers.
    function expelBankRoller(address bankRoller) external onlyOwner {
        uint currentDeposit = individualDeposit[bankRoller];
        require(currentDeposit > 0, "This address has zero balance");
        uint contractBalance = ERC223Interface(trc20Token).balanceOf(address(this));
        require((contractBalance * currentDeposit * 99) / 99 == contractBalance * currentDeposit, "Multiply operation is not safe");
        uint withdrawAmount = contractBalance * currentDeposit * 99 / totalDeposit / 100; // 1% for reserve - cover lockInBet, gas, etc
        removeDepositor(bankRoller);
        setMaxCapIndividualDeposit(bankRoller, 0);
        totalDeposit = totalDeposit - currentDeposit;
        individualDeposit[bankRoller] = 0;
        ERC223Interface(trc20Token).transfer(bankRoller, withdrawAmount);
        emit BankRollerWithdraw(bankRoller, 100, withdrawAmount);
    }
