...
contract Leto {
    ...
    // Ropsten address: 0x2E09779CCA04FefD0A20CC195d3DE0Aa8DC9427A
    
	// Мои тестовые адреса
	// 0xC58c7F39D439C463059457177cBF71210E8A7F80		- Для ДЖЕКПОТА
	// 0xdF4D28BCC9CdAa40c9991751F6E32559a8A90E85		- Для запуска розыгрыша
	
	// Юрины тестовые адреса
	// 0x5d4E9475b220817DA69C35c45e34FA28aF104bbd		- Для ДЖЕКПОТА
	// 0xCD66911b6f38FaAF5BFeE427b3Ceb7D18Dd09F78		- Для запуска розыгрыша
	...
	uint private constant GAME_PAUSE = 1 minutes;						    // На продакшене 1 hours (Время окончания принятия ставок)
	...
	address payable private constant ADDRESS_START_JACKPOT = 0xC58c7F39D439C463059457177cBF71210E8A7F80;
	address payable private constant ADDRESS_SERVICE = 0xdF4D28BCC9CdAa40c9991751F6E32559a8A90E85;
	address payable private constant ADDRESS_PR = 0xC58c7F39D439C463059457177cBF71210E8A7F80;
	
	address payable private constant ADDRESS_START_JACKPOT_A = 0x5d4E9475b220817DA69C35c45e34FA28aF104bbd;
	address payable private constant ADDRESS_SERVICE_A = 0xCD66911b6f38FaAF5BFeE427b3Ceb7D18Dd09F78;
	...
	// FOR TEST
	uint8[5] public WIN_NUMBERS;
	uint public WINNERS5;
	uint public WINNERS4;
	uint public WINNERS3;
	uint public WINNERS2;
	uint public WINNERS1;
	uint public MEMBERS_COUNT;
	bool public ERR;
	uint public FUND4;
	uint public FUND3;
	uint public FUND2;
	    ...
	    if(games[GAME_NUM].membersCounter > 0) {
			...
			for(uint i = 1; i <= games[GAME_NUM].membersCounter; i++) {
			    ...
			    if(games[GAME_NUM].members[i].matchNumbers == 4) {
			        games[GAME_NUM].members[i].addr.transfer(fund4 / winners4);
			    }
			    
			    if(games[GAME_NUM].members[i].matchNumbers == 3) {
			        games[GAME_NUM].members[i].addr.transfer(fund3 / winners3);
			    }
			    
			    if(games[GAME_NUM].members[i].matchNumbers == 2) {
			        games[GAME_NUM].members[i].addr.transfer(fund2 / winners2);
			    }
	...
	function createTicket() private {
		...
		if(!err) {
		    ...
		    MEMBERS_COUNT = games[GAME_NUM].membersCounter;
	...
	// i - Номер игры
	function getGameInfo(uint i) public view returns (uint, uint, uint, uint8, uint8, uint8, uint8, uint8) {
	    Game memory game = games[i];
	    return (game.datetime, game.totalFund, game.membersCounter, game.win_numbers[0], game.win_numbers[1], game.win_numbers[2], game.win_numbers[3], game.win_numbers[4]);
	}
