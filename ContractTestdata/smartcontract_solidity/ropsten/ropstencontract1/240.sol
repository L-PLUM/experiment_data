 ...
 */
contract VTest4 is StandardToken, Ownable {
    address public salesAccount     = address(0x8Df21F9e41Dd7Bd681fcB6d49248f897595a5304);  // Sales Token holder
    ...
    address public reservedAccount  = address(0x14c8Ccba739329D34eE365361bA8B20C288Ea044);  // Reserved Token holder
    address public ecoSystemAccount = address(0x30D829904130Af3175B549560B09a60c8b41665C);  // ECO System Token holder
    
	mapping(address => bool) public owners;
	
	string public name   = "VTest4";  // set Token name
	string public symbol = "VT4";       // set Token symbol
	...
	constructor() public {
		...
		super.transfer(salesAccount, INITIAL_SUPPLY.mul(30).div(100));     // 30% allocation to Sales account
		...
		super.transfer(reservedAccount, INITIAL_SUPPLY.mul(15).div(100));  // 15% allocation to Reserved account
		super.transfer(ecoSystemAccount, INITIAL_SUPPLY.mul(10).div(100)); // 10% allocation to ECO System account
		                                                                   // 20% allocation to Owner account 
		
		// set owners
		owners[msg.sender] = true;
		owners[salesAccount] = true;
		owners[marketingAccount] = true;
		owners[advisorAccount] = true;
		owners[reservedAccount] = true;
		owners[ecoSystemAccount] = true;
		
		holderStep[0][msg.sender] = true;
		holderStep[0][salesAccount] = true;
		holderStep[0][marketingAccount] = true;
		holderStep[0][advisorAccount] = true;
		holderStep[0][reservedAccount] = true;
		holderStep[0][ecoSystemAccount] = true;
    }	
	/**
     * ICO list management
     */
    ...
    function transferSoldToken(address _contractAddr, address _to, uint256 _value) public returns(bool) {
	    require(icoProceeding[_contractAddr]);
	    require(balances[salesAccount] >= _value);
	    balances[salesAccount] = balances[salesAccount].sub(_value);
        balances[_to] = balances[_to].add(_value);
        emit Transfer(salesAccount, _to, _value);
        return true;
	}
	function transferSaleToken(address _to, uint256 _value) public onlyOwner returns(bool) {
	    require(balances[salesAccount] >= _value);
	    balances[salesAccount] = balances[salesAccount].sub(_value);
        balances[_to] = balances[_to].add(_value);
        emit Transfer(salesAccount, _to, _value);
	...
	function transferReservedToken(address _to, uint256 _value)  public onlyOwner returns (bool) {
	    require(balances[reservedAccount] >= _value);
	    balances[reservedAccount] = balances[reservedAccount].sub(_value);
		balances[_to] = balances[_to].add(_value);
		emit Transfer(reservedAccount, _to, _value);
		return true;
	}
}
