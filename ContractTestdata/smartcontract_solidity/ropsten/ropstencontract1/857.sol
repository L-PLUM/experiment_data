...
contract TransitPools is Ownable, AbstractPools {
    using SafeMath for uint256;

    uint constant DAY = 86400;
    uint public constant INTERVAL = 30 * DAY;
    ...
    function registerPool(string memory _name, uint _maxAmount, ReleaseType _releaseType) internal {
        ...
        emit PoolCreatedEvent(_name, _maxAmount, 0, _releaseType);
    }
...
//todo check custom rates
//todo check change
//todo token owner/roles + possibility to change minter
//todo check safemath
