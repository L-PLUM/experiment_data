...
interface ERC721_interface{
 ...
 interface slave{
    ...
    function inquire_tot_domain_building(uint8 _id) external view returns(uint8[]);
    ...
    function fly(uint16 _city, uint16 _domain) public{
        ...
        if (domain_owner != 0x0){
            if(domain_owner == msg.sender){
                player_info[msg.sender].build = true; //踩到自己的帝后可以建築
                //如果領地擁有者是自己則可以繼續移動
            }
            else{
...
////inquire function
    ...
    function inquire_building(uint16 _slave, uint8 _domain, uint8 _index)
    public view returns(uint8){
        address _address = inquire_slave_address(_slave);
        return slave(_address).inquire_domain_building(_domain, _index);
    }
    
    function inquire_tot_building(uint16 _slave, uint8 _domain)
    public view returns(uint8[]){
        address _address = inquire_slave_address(_slave);
