...
contract Network is Withdrawable, Utils2, NetworkInterface {
    ...
    function listPairForReserve(address reserve, TRC20 token, bool ethToToken, bool tokenToEth, bool add)
        public onlyAdmin
    {
        require(isReserve[reserve]);
    ...
    function setKyberProxy(address networkProxy) public onlyAdmin {
        ...
        emit KyberProxySet(networkProxy, msg.sender);
    }
    ...
    {
        (result.reserve1, result.rateSrcToEth) = searchBestRate(src, TOMO_TOKEN_ADDRESS, srcAmount);
        result.weiAmount = calcDestAmount(src, TOMO_TOKEN_ADDRESS, srcAmount, result.rateSrcToEth);

        (result.reserve2, result.rateEthToDest) = searchBestRate(TOMO_TOKEN_ADDRESS, dest, result.weiAmount);
        result.destAmount = calcDestAmount(TOMO_TOKEN_ADDRESS, dest, result.weiAmount, result.rateEthToDest);
    ...
    event KyberTrade(address srcAddress, TRC20 srcToken, uint srcAmount, address destAddress, TRC20 destToken,
        uint destAmount);
    ...
    function tradeFee(TradeInput memory tradeInput) internal returns(uint) {
      ...
      require(expectedRate >= tradeInput.minConversionRate);
        ...
        internal view returns(uint actualSrcAmount, uint weiAmount, uint actualDestAmount)
    {
        if (rateResult.destAmount > maxDestAmount) {
            ...
            weiAmount = calcSrcAmount(TOMO_TOKEN_ADDRESS, dest, actualDestAmount, rateResult.rateEthToDest);
            actualSrcAmount = calcSrcAmount(src, TOMO_TOKEN_ADDRESS, weiAmount, rateResult.rateSrcToEth);
    ...
    function validateTradeInput(TRC20 src, uint srcAmount, TRC20 dest, address destAddress)
        internal
        view
        returns(bool)
    {
