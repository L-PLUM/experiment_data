...
contract Fizzy is Ownable {
    ...
    */
    function addNewInsurance(
        ...
        _addNewInsurance(flightId, premium, indemnity, productId, limitArrivalTime, conditions, address(0));
    }
    ...
    */
    function setFlightLandedTODOTEMPNAMETOBEREPLACEDDANGERWARNINGINFO(
        bytes32 flightId,
    ...
    function setFlightStatus(
        ...
        for (uint i = 0; i < insuranceList[flightId].length; i++) {
            ...
            if (insurance.status == InsuranceStatus.Open) {
                ...
                insurance.status = newInsuranceStatus;
    ...
    */
    ...
    function _addNewInsurance (
        ...
        emit InsuranceCreation(flightId, productId, premium, indemnity, conditions, compensationAddress);
    }
...
contract FizzyCrypto is Fizzy {
    ...
    {
        _checkSignature(flightId, productId, limitArrivalTime, conditions, premium, taxes, indemnity, timestampLimit, signature);

        require(premium >= taxes, "Taxes must be included in the premium.");
        require(premium == msg.value,
            "The amount sent does not match the price of the order if it does not match the amount variable.");
