...
contract EasyToken is ERC20, Ownable, usingOraclize {
	...
	function executeBuyQueue() private {
		...
		for(uint256 i=1; i<=buyLast; i++ ) {
			...
			numToken = bValue.mul(ethEaxRate).mul((10**decimals)); 
			// ethEaxRate from __callback!
	...
	function __callback(bytes32 myid, string memory result) public
	{
        ...
        // Calculating rate
		ethEaxRate = parseInt(result) * 100;
		...
		uint udot = uint(dot);
		string memory cents = substring(result, udot+1, udot+3); // end is not included
		ethEaxRate = ethEaxRate + parseInt(cents);
