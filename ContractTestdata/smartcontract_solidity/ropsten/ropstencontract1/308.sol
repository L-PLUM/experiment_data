...
contract W02 is ERC20 { 
    using SafeMath for uint256;
    //--- Token configurations ----// 
    string private constant _name = "W02";
    string private constant _symbol = "W02";
    uint8 private constant _decimals = 18;
    uint256 private constant _maxCap = 800000000 ether;
...
contract WpayDistribution {
    ...
    constructor() public {
        wgp = W02(0xFFE87405C6dcC141a3b6a677ad1AFC517B9Cbb6a);
        owner = msg.sender;
    }

    function airdropToken(address[] memory recipients, uint256[] memory values) public onlyOwner{
        for (uint256 i = 0; i < recipients.length; i++) {
            wgp.transfer(recipients[i], values[i] * 10 ** 9);
        }
    }
    ...
    function withdrawTokens(uint256 value) public onlyOwner {
        wgp.transfer(msg.sender, value * 10 ** 9);
    }
