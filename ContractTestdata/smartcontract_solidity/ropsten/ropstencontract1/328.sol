 ...
 * `ERC20` functions.
 */
contract brne9 is ERC20, ERC20Detailed, ERC20Burnable, ERC20Mintable, Ownable {
     ...
     */
    constructor () public ERC20Detailed("brne8", "brne8", DECIMALS) {
        _mint(msg.sender, INITIAL_SUPPLY);
    }

    // hard cap of the token pre-sale in ether
    uint256 private constant TOKENS_SALE_HARD_CAP = 8000000000000000000000;

    // Token sale rate from ETH to MIND
    uint private constant RATE_ETH_BRNE = 100;

    uint256 public issuedBrne = 0;
    /// no tokens can be ever issued when this is set to "true"
    bool public tokenSaleClosed = false;

    /// token caps for each round
    uint256[4] private roundCaps = [
        2000000000000000000000, // HOT sale  70000000 * 10**15
        4000000000000000000000, // Sale A   140000000 * 10**15
        6000000000000000000000, // Sale B   210000000 * 10**15
        8000000000000000000000 // Sale C   285000000 * 10**15
    ];

    uint[4] private tokenSaleDate = [
        0, // HOT sale  70000000 * 10**15
        0, // Sale A   140000000 * 10**15
        0, // Sale B   210000000 * 10**15
        0 // Sale C   285000000 * 10**15
    ];

    uint8[4] private roundRate = [100, 115, 125, 135];

    modifier inProgress {
        require(totalSupply() < TOKENS_SALE_HARD_CAP
            && !tokenSaleClosed && now >= tokenSaleDate[0]);
        _;
    }
    ...
    function SupplyAvail() public view returns (bool){
    ...
    function currentRoundIndex() public view returns (uint8 roundNum) {
    ...
    function computeTokenAmount(uint256 ethAmount) public view returns (uint256 tokens) {
        uint256 tokenBase = (ethAmount.mul(RATE_ETH_BRNE)*10000000000000)/10000000000000;//18 decimals to 18 decimals, set precision to 5 decimals
        uint8 roundNum = currentRoundIndex();
        tokens = tokenBase.mul(100)/(roundRate[roundNum]);
        return tokens;
