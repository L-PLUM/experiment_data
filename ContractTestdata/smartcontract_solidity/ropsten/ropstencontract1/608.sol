...
contract UpcityMarket is BancorFormula, Uninitialized, Restricted, IMarket {
	...
	uint32 private constant CONNECTOR_WEIGHT = 660000;
	...
	function init(
		...
		yesterday = uint64(block.timestamp);
		_bancorInit();
		_init();
	}
	...
	function getTokens()
		...
		tokens[0] = _tokenAddresses[0];
		tokens[1] = _tokenAddresses[1];
		tokens[2] = _tokenAddresses[2];
	}
	...
	function getPrices()
		...
		prices[0] = _getTokenPrice(
			_tokens[_tokenAddresses[0]].funds,
			_tokens[_tokenAddresses[0]].supply);
		prices[1] = _getTokenPrice(
			_tokens[_tokenAddresses[1]].funds,
			_tokens[_tokenAddresses[1]].supply);
		prices[2] = _getTokenPrice(
			_tokens[_tokenAddresses[2]].funds,
			_tokens[_tokenAddresses[2]].supply);
	}
	...
	function getSupplies()
		...
		supplies[0] = _tokens[_tokenAddresses[0]].supply;
		supplies[1] = _tokens[_tokenAddresses[1]].supply;
		supplies[2] = _tokens[_tokenAddresses[2]].supply;
	}
	...
	function getBalances(address owner)
		...
		balances[0] = _tokens[_tokenAddresses[0]].balances[owner];
		balances[1] = _tokens[_tokenAddresses[1]].balances[owner];
		balances[2] = _tokens[_tokenAddresses[2]].balances[owner];
	}
	...
	function transfer(
		...
		_transfer(_tokens[_tokenAddresses[0]], from, to, amounts[0]);
		_transfer(_tokens[_tokenAddresses[1]], from, to, amounts[1]);
		_transfer(_tokens[_tokenAddresses[2]], from, to, amounts[2]);
	}
	...
	function mint(address to, uint256[NUM_RESOURCES] calldata amounts)
		...
		_touch();
		_mint(_tokens[_tokenAddresses[0]], to, amounts[0]);
		_mint(_tokens[_tokenAddresses[1]], to, amounts[1]);
		_mint(_tokens[_tokenAddresses[2]], to, amounts[2]);
	}
	...
	function _touch() private {
		...
		if (_now > yesterday && _now - yesterday >= 86400) {
			_tokens[_tokenAddresses[0]].priceYesterday = _getTokenPrice(
				_tokens[_tokenAddresses[0]].funds,
				_tokens[_tokenAddresses[0]].supply);
			_tokens[_tokenAddresses[1]].priceYesterday = _getTokenPrice(
				_tokens[_tokenAddresses[1]].funds,
				_tokens[_tokenAddresses[1]].supply);
			_tokens[_tokenAddresses[2]].priceYesterday = _getTokenPrice(
				_tokens[_tokenAddresses[2]].funds,
				_tokens[_tokenAddresses[2]].supply);
			yesterday = _now;
		}
