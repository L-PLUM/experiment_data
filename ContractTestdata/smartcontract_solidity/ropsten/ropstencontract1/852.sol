...
contract FiveElementsAdministration is QuickSort,Ceil,Abssub{
    ...
    function Results(uint RealPriceA,uint RealPriceB,uint RealPriceC,uint RealPriceD,uint RealPriceE,bool KeepPrevData,uint NewEntryPrice,uint NewPrizePool,uint NewSubmissionPeriod){
        ...
        if (L>0){
        for (uint k=0;k<L;k++){
            Error.push(1000000*Weights[0]*AbsSub(GuessesA[k],Ans[0])/Ans[0]+1000000*Weights[1]*AbsSub(GuessesB[k],Ans[1])/Ans[1]+1000000*Weights[2]*AbsSub(GuessesC[k],Ans[2])/Ans[2]+1000000*Weights[3]*AbsSub(GuessesD[k],Ans[3])/Ans[3]+1000000*Weights[4]*AbsSub(GuessesE[k],Ans[4])/Ans[4]);
        }
        Error=sort(Errors);
