...
contract FiveElementsAdministration is QuickSort,Ceil,Abssub{
    ...
    function Results(uint RealPriceA,uint RealPriceB,uint RealPriceC,uint RealPriceD,uint RealPriceE,bool KeepPrevData,uint NewEntryPrice,uint NewPrizePool,uint NewSubmissionPeriod){
        ...
        if (L>0){
        for (uint k=0;k<L;k++){
            Errors.push(1000000*Weights[0]*AbsSub(GuessesA[k],Ans[0])/Ans[0]+1000000*Weights[1]*AbsSub(GuessesB[k],Ans[1])/Ans[1]+1000000*Weights[2]*AbsSub(GuessesC[k],Ans[2])/Ans[2]+1000000*Weights[3]*AbsSub(GuessesD[k],Ans[3])/Ans[3]+1000000*Weights[4]*AbsSub(GuessesE[k],Ans[4])/Ans[4]);
        }
        Error=sort(Errors);
    ...
    function UserJoin(address User,uint Value,uint GuessA,uint GuessB,uint GuessC,uint GuessD,uint GuessE){
        ...
        Users.push(User);
        CanJoin.push(User);
        GuessesA.push(GuessA);
        GuessesB.push(GuessB);
        GuessesC.push(GuessC);
        GuessesD.push(GuessD);
        GuessesE.push(GuessE);
        Guessed.push(true);
