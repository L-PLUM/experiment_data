...
/**
 * @title PhotoToken
 * Photo - a contract for non-fungible photos.
 */
contract PhotoToken is ERC721Full, Ownable, Pausable {
  constructor(address payable _feesTo) ERC721Full("Photo", "PHO") public {
    pause();
  ...
  function baseTokenURI() public pure returns (string memory) {
    return "http://13.58.53.6/api/photo/";
  }
  ...
  function mintPhoto(uint256 tokenId, string memory hash) public onlyOwner {
    require(tokenId <= 100000, "You can only mint 100k photos");
  ...
  function updateCurrentPrice() internal {
    currentPrice = getCurrentPriceUpdate(currentPrice, totalBought);
    totalBought = totalBought + 1;
  }

  function getCurrentPriceUpdate(uint256 _currentPrice, uint256 _totalBought) internal view returns (uint256) {
    if (_totalBought < 10000 - 1) {
      return _currentPrice * 10005 / 10000;
    }

    if (_totalBought < 100000 - 1) {
      return currentPrice * 1005 / 1000;
    }

    return 13330000000000000000;
  }
  ...
  function buyToken(uint256 tokenId) public payable {
    ...
    require(price == msg.value, "You need to send the exact price");
  ...
  //function getCurrentPriceForBulk(uint total) public returns (uint256) {
  //  uint256 _totalPrice = currentPrice;
  //  #for...
  //    _currentPrice = getCurrentPriceUpdate(_currentPrice, _totalBought);
  //    _totalPrice += _currentPrice;
  //   _totalBought++;
  //  return _totalPrice;
  //}

  //function buyTokenBulk(uint256 total) whenNotPaused public payable {
  //    require(total < 51, "You can't buy more than 50 tokens");
  //  # for...
  //    require(ownerOf(tokenId) == owner(), "You can't buy this token directly from the contract");
  
  //  price = getCurrentPriceForBulk(uint256 total);
...
contract BitAirtExchange is Ownable {
  PhotoToken private photoToken;
  address payable private feesTo;

  constructor(PhotoToken _photoToken, address payable _feesTo) public {
  ...
  mapping(uint256 => mapping(address => uint256)) private acquisitionEscrows; //tokenId to mapping of acquirer to bid

  function startPhotoAcquisition(uint256 tokenId) payable public {
    acquisitionEscrows[tokenId][msg.sender] = msg.value;

    emit AcquisitionStarted(tokenId, msg.sender, msg.value);
  }

  function cancelPhotoAcquisition(uint256 tokenId) public {
    require(acquisitionEscrows[tokenId][msg.sender] > 0);
    acquisitionEscrows[tokenId][msg.sender] = 0;

    emit AcquisitionCancelled(tokenId, msg.sender);
  }

  function acceptPhotoAcquisition(uint256 tokenId, address acquirer) public {
    require(acquisitionEscrows[tokenId][acquirer] > 0);
    require(photoToken.ownerOf(tokenId) == msg.sender);

    uint256 value = acquisitionEscrows[tokenId][msg.sender];
    acquisitionEscrows[tokenId][msg.sender] = 0;
    photoToken.transferFrom(msg.sender, acquirer, tokenId);

    uint256 fees = value * 5 / 100;
