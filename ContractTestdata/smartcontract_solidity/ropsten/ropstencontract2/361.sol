...
contract ConvertButterCoinToEther is owned, ConverterRole{
    ...
    constructor() public
     {
         ...
         buttercoinToEth_conversion_rate = 0;
     }
    ...
    function convertButterCointoEther(uint256 value) public onlyConverter{
        require(value <= ButterCoin.balanceOf(address(msg.sender)));
        ButterCoin.safeTransferFrom(ButterCoin,address(msg.sender),value);
        if(buttercoinToEth_conversion_rate > 0)
        {
            uint256 equivalent_wei = value.mul(getConversionRate());
            require(address(this).balance >= equivalent_wei);
            msg.sender.transfer(equivalent_wei);
        }
    }
     function depositEther() public payable onlyOwner{
        // Owner will deposit Ether
