...
Further enhancedments have been made for this third-generation including enhanced anti-fraud measures, as well as a contingency plan to ensure that in the event of an exchange being compromised, that there is a facility to prevent the damage hurting the Tratok ecosystem.


@version "1.2"
@developer "Tratok Team"
@date "25 June 2019"
@thoughts "356 lines that will change the travel and tourism industry! Good luck!"
*/
...
This function determines distributes tratok to multiple addresses and is designed for the airdrop.
    ...
    function airDropTratok(address[] _destinations, uint256[] _values) public onlyOwner
    returns (uint256) {
        uint256 i = 0;
...
/*        
This function can be used to lock accounts in bulk in the event of fraud commited and being committed during promotion programs. The m

     @param _targets The address of the accounts which will be locked.
     @return The number of loop cycles
     */        
    
    function massLock(address[] _targets)
   external onlyOwner returns (uint256) {
        uint256 i = 0;
        while (i < _targets.length) {
          lockAccountFromSendingTratok(_targets[i]);
           i += 1;
        }
        return(i);
    }
...
/*    
This function can be used to unlock accounts in bulk in the event of fraud commited and being resolved during promotion programs.

     @param _targets The address of the accounts which will be unlocked.
     @return The number of loop cycles
     */        
    
    function massUnLock(address[] _targets)
   external onlyOwner returns (uint256) {
        uint256 i = 0;
        while (i < _targets.length) {
           unlockAccountFromSendingTratok(_targets[i]);
           i += 1;
        }
        return(i);
    }        
       ...
       /*
This function locks the account  from sending Tratok. This measure is taken to prevent fraud and to recover Tratok in the event that an excahnge is hacked.
     @param _target The address of the accounts which will be locked.
     @return The number of loop cycles
     */    
     
   function lockAccountFromSendingTratok(address _target) public onlyOwner returns(bool){
...
This function unlocks accounts tratok to allow them to send Tratok.
     ...
     @return The number of loop cycles
     */    
      function unlockAccountFromSendingTratok(address _target) public onlyOwner returns(bool){
      require(_target != address(0));
      require(frozenAccounts[_target]);
...
This function is an emergency failsafe that allows Tratok to be reclaimed in the event of an exchange or partner hack. This contingency is meant as a last resort to protect the integrity of the ecosystem in the event of a security breach. It can only be executed by the holder of the custodian keys.
     ...
     @param _value The amount of Tratok tokens that will be transfered.
     */    
...
contract Tratok is StandardToken {
   ...
   /*
    * Method to ensure that the public cannot accidentally send ETH to the contract address. Any such funds mistakes will be returned.
    */ 
