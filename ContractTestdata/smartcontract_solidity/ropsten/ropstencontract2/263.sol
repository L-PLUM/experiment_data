...
contract GenieIdentity is Ownable{ 
    ...
    //
    function CheckClaim(Identity _identity, uint256 claimType, address contractAddress) public returns (bool claimValid) {
        Storage storageSpace = Storage(_storageSpace);
        ClaimVerifier verifiers = ClaimVerifier(contractAddress);
        return verifiers.checkClaim(_identity, claimType);
    }
    
    function ClaimIsValid(Identity _identity, uint256 claimType, address contractAddress) public view returns (bool claimValid) {
        Storage storageSpace = Storage(_storageSpace);
        ClaimVerifier verifiers = ClaimVerifier(contractAddress);
        return verifiers.claimIsValid(_identity, claimType);
    }
