...
contract PAYMENT is Ownable {
    ...
    public {
        ...
        _providers[creosafeContract] = Provider(creosafeContract, false, 0, 0, 0, 0);
    }
    ...
    function addCustomer(address customerAddress) public available isCreosafeAdmin(msg.sender) isCustomerNotAdded(customerAddress){
        _customers[customerAddress] = Customer(customerAddress, false, 0, 0);
    }

    function updateCustomerBalance(address customerAddress, uint weiAmount, uint tokenAmount) public available isCreosafeAdmin(msg.sender) isCustomerAdded(customerAddress){
        require(tokenAmount >= 0);
        require(weiAmount >= 0);
        require(tokenAmount.add(weiAmount) > 0 );
