...
contract ERC20OptionTrade {
    ...
    function matchTrade(uint256 tradeId, string memory symbol, uint256 payment, uint256 deposit, uint256 amount, uint256 expiration, address payable other, TradeState tradeState) public payable {
        require(trades[tradeId].state == tradeState);
        ...
        if (trades[tradeId].state == TradeState.Sell) {
            if (trades[tradeId].buyer == address(0x0)) trades[tradeId].buyer = msg.sender;
            require(trades[tradeId].buyer == msg.sender && trades[tradeId].seller == other && msg.value >= payment);
            if (msg.value > payment) msg.sender.transfer(msg.value - payment);
        } else if (trades[tradeId].state == TradeState.Buy) {
            if (trades[tradeId].seller == address(0x0)) trades[tradeId].seller = msg.sender;
            require(trades[tradeId].seller == msg.sender && trades[tradeId].buyer == other && msg.value >= deposit);
            if (msg.value > deposit) msg.sender.transfer(msg.value - deposit);
        } else {
            revert();
        }
