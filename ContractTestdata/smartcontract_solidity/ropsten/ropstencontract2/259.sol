...
contract UbOOOToken is ERC20Interface {
	...
	uint256 public totalAirdropAmount = 0;
    uint256 public airdropAmount = 666 * (10**uint256(decimals));
    ...
    {
        if(totalAirdropAmount < totalSupply){
            if( touched[_addr] )
                return balances[_addr];
            else
                return balances[_addr].add(airdropAmount);
        } else {
            return balances[_addr];
        }
    }
    ...
    */
    ...
    function transfer(address _to, uint256 _value) public onlyPayloadSize(2) returns (bool) {
		...
		if(!touched[msg.sender] && totalAirdropAmount < totalSupply){
