...
contract ERC223ReceivingContract {
    ...
    byte public expectedByteValue; 
    
    bool public expectedValueReceived;
    
    event expectedByteValueChanged(
        byte newValue
        );
     
    function changeExpectedByteValue (byte _newValue) onlyOwner public returns (bool success){
        expectedByteValue = _newValue; 
        emit expectedByteValueChanged(expectedByteValue);
        return true;
    }

    event toekensReceived(
            address tokensContract,
            address from, 
            uint value, 
            bytes data
        );
    ...
    function tokenFallback(address _from, uint _value, bytes calldata _data) external returns(bool success){
        ...
        if(_data[0]==expectedByteValue){
            expectedValueReceived = true;
        } else {
            expectedValueReceived = false;
        }
        
        bytesValue = _data; 
        emit toekensReceived(msg.sender, _from, _value, _data);
