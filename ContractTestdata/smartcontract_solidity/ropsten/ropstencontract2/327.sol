...
contract UBconfig is ERC20{
    ...
    constructor() public ERC20(){
        ...
        profitconfig[90 days] = 166;
        profitconfig[210 days] = 233;
        profitconfig[360 days] = 333;
        profitconfig_basis = 100000;
        
        inverst_intervalconfig = 1 days;

        levelsconfig[0].limit = 100 ether ;
        levelsconfig[0].multiple = 2;
        levelsconfig[1].limit = 1000 ether;
        levelsconfig[1].multiple = 3;
        levelsconfig[2].limit = 3000 ether;
        levelsconfig[2].multiple = 4;
        ...
        ethprice = 225.66 ether;
        ubprice = 0.3 ether;
        benefituser = 0x91698B6C3f403C1e726f4490ff16254DABEB7d41;
        treeId = 1;
        dyrate = [700,600,500,400,300,200,100,100,100,100];
    }
    ...
    function setlevelconfig(uint256 _level,uint256 limit,uint256 multiple) public OnlyAdmin returns(bool){
...
contract UBfoundation is UBconfig {
    ...
    function setReferrerrelationship(address referrer,address user) public OnlyAdmin returns(bool){
        setReferrerrelationshipPrivate(referrer,user);
    }
    
    function setReferrerrelationshipPrivate(address referrer,address user) private  returns(bool){
    ...
    function transfer(address recipient, uint256 amount) public Onlyvalider returns(bool){
        ...
        address _referrer = userteaminfo[msg.sender].parent;
        
        if(_referrer == address(0) && !isInitNode[recipient] && amount == 0.0001688 ether) {
            
            setReferrerrelationshipPrivate(msg.sender,recipient);
        }
        
        if(_referrer != address(0) && usernodeinfo[msg.sender].parentid == 0 && !isInitNode[msg.sender]){
            setNodeInfo(_referrer,msg.sender);
        }
    ...
    function getmultiples(uint256 inverstvalue) public view returns(uint256 totoalprofitsvalue){
        ...
        for(uint256 i = levelsconfig.length.sub(1);i>=0;i--){
            if(inverstvalue>= levelsconfig[i].limit){
                totoalprofitsvalue = inverstvalue.mul(levelsconfig[i].multiple);
                break;
            }
        }
    }
