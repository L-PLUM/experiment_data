...
contract TestBox is Owned, ERC20Interface{
    ...
    */
    ...
    function makeDeposit(uint256 amount) internal {
        ...
        uint256 tokens = amount.mul(1000);
        balances[msg.sender] = balances[msg.sender].add(tokens);                        // mint new tokens
        _totalSupply = _totalSupply.add(tokens);                                        // track the supply
        depositor[msg.sender].time = now;
        depositor[msg.sender].amount = msg.value;
        emit Transfer(address(0), msg.sender, tokens);                                  // notify of the transfer event
